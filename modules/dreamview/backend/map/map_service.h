/******************************************************************************
 * Copyright 2017 The JmcAuto Authors. All Rights Reserved.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 * http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 *****************************************************************************/

/**
 * @file
 */

#ifndef MODULES_DREAMVIEW_BACKEND_MAP_MAP_SERVICE_H_
#define MODULES_DREAMVIEW_BACKEND_MAP_MAP_SERVICE_H_

#include <string>
#include <vector>

#include "boost/thread/locks.hpp"
#include "boost/thread/shared_mutex.hpp"

#include "modules/dreamview/proto/simulation_world.pb.h"
#include "modules/map/pnc_map/pnc_map.h"
#include "third_party/json/json.hpp"
#include "modules/common/proto/geometry.pb.h"

/**
 * @namespace jmc_auto::dreamview
 * @brief jmc_auto::dreamview
 */
namespace jmc_auto {
namespace dreamview {
 // using jmc_auto::control::PadMessage;
  using jmc_auto::hdmap::ParkingSpaceInfoConstPtr;

class MapService {
 public:
  explicit MapService(bool use_sim_map = true);

  inline double GetXOffset() const {
    return x_offset_;
  }
  inline double GetYOffset() const {
    return y_offset_;
  }


//  bool isPointInRect(jmc_auto::common::PointENU &point0, jmc_auto::common::PointENU &point1, jmc_auto::common::PointENU &point2, jmc_auto::common::PointENU &point3, jmc_auto::common::PointENU &point4);
//  std::vector<ParkingSpaceInfoConstPtr> CheckPointInParkingSpace(jmc_auto::common::PointENU &point) const;
  bool CheckPointInParkingSpace(jmc_auto::common::PointENU &point, std::string &id, std::vector<jmc_auto::common::PointENU> &points) const;

  bool getParkingspaceBy_id(const std::string &id, std::vector<jmc_auto::common::PointENU> &points) const;



  void CollectMapElementIds(const jmc_auto::common::PointENU &point,
                            double raidus, MapElementIds *ids) const;

  bool GetPathsFromRouting(const jmc_auto::routing::RoutingResponse &routing,
                           std::vector<jmc_auto::hdmap::Path> *paths) const;

  // The returned value is of a hdmap::Map proto. This
  // makes it easy to convert to a JSON object and to send to the
  // javascript clients.
  hdmap::Map RetrieveMapElements(const MapElementIds &ids) const;

  bool GetPoseWithRegardToLane(const double x, const double y, double *theta,
                               double *s) const;

  // Get a point on the map to serve as dummy start point of SimControl
  bool GetStartPoint(jmc_auto::common::PointENU *start_point) const;

  /**
   * @brief The function fills out proper routing lane waypoint
   * from the given (x,y) position.
   * @param x position
   * @param y position
   * @param laneWayPoint RoutingRequest's lane waypoint
   * @param flag_appmode when parking set to 1
   * @return True if the lane waypoint is filled successfully.
   */
  bool ConstructLaneWayPoint(const double x, const double y,
                             routing::LaneWaypoint *laneWayPoint, const short int flag_appmode) const;


  // Reload map from current FLAGS_map_dir.
  bool ReloadMap(bool force_reload);

  bool ConstructParkingLaneWayPoint(const double x, const double y,
                             jmc_auto::routing::RoutingRequest *routing_request, const short int flag_appmode) const;

  size_t CalculateMapHash(const MapElementIds &ids) const;

 private:
  void UpdateOffsets();
  bool GetNearestLane(const double x, const double y,
                      jmc_auto::hdmap::LaneInfoConstPtr *nearest_lane,
                      double *nearest_s, double *nearest_l) const;

  bool CreatePathsFromRouting(const routing::RoutingResponse &routing,
                              std::vector<jmc_auto::hdmap::Path> *paths) const;

  bool AddPathFromPassageRegion(const routing::Passage &passage_region,
                                std::vector<jmc_auto::hdmap::Path> *paths) const;

  static const char kMetaFileName[];

  const bool use_sim_map_;
  const hdmap::HDMap *HDMap() const;
  // A downsampled map for dreamview frontend display.
  const hdmap::HDMap *SimMap() const;
  bool MapReady() const;

  double x_offset_ = 0.0;
  double y_offset_ = 0.0;

  // RW lock to protect map data
  mutable boost::shared_mutex mutex_;
};

}  // namespace dreamview
}  // namespace jmc_auto

#endif  // MODULES_DREAMVIEW_BACKEND_MAP_MAP_SERVICE_H_
