...
using jmc_auto::control::DrivingMotion;
...
// 新增继续和暂停功能
void HMIWorker::ChangeToDrivingAction(const std::string& mode) {

  control::PadMessage pad;
  if (Chassis::COMPLETE_AUTO_DRIVE){
    pad.set_motion(DrivingMotion::CONTINUE);
    if (mode == "Pause"){
      pad.set_motion(DrivingMotion::PAUSE);
    }
    else if(mode == "keepOn"){
      pad.set_motion(DrivingMotion::CONTINUE);
    }
    else {
      AFATAL << "Unknown action to change driving Action to " << mode;
    }
//      pad.clear_motion();
  }else{
    pad.set_motion(DrivingMotion::CONTINUE);
  }
  AINFO << "the pause/keepon pad is: " <<pad.DebugString();
  AdapterManager::FillPadHeader("HMI", &pad);
  AdapterManager::PublishPad(pad);
  return;
}
