...
void HMI::RegisterMessageHandlers() {
  ...
  websocket_->RegisterMessageHandler(
      ...
      [this](const Json &json, WebSocketHandler::Connection *conn) {
        ...
        AERROR << "Your Json is: " << json;
        AERROR << "Your conn is" << conn;
        std::string new_mode;
  ...
  websocket_->RegisterMessageHandler(
      ...
      [this](const Json &json, WebSocketHandler::Connection *conn) {
        ...
        std::string new_mode;
        if (JsonUtil::GetStringFromJson(json, "New_mode", &new_mode)) {
          if(new_mode == "Pause" || "keepOn"){
            HMIWorker::instance() -> ChangeToDrivingAction(new_mode);
          } else {
            AERROR << "Unknown driving action " << new_mode;
          }
        } else {
          AERROR << "Truncated ChangeDrivingAction request.";
        }
      });
  ...
  websocket_->RegisterMessageHandler(
      ...
      [this](const Json &json, WebSocketHandler::Connection *conn) {
        ...
        AWARN << "Your changemap Json is " << json;
        AWARN << "Your changemap conn is" << conn;
        std::string new_map;
