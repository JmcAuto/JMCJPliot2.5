...
void SimulationWorldService::Update() {
  if (to_clear_) {
    ...
    AdapterManager::GetPerceptionRoundviewObstacles()->ClearData();
  ...
  UpdateWithLatestObserved("PerceptionRoundviewObstacles",
                           AdapterManager::GetPerceptionRoundviewObstacles());
...
void SimulationWorldService::UpdatePlanningTrajectory(
  ...
  // the parkingstatus "SUCCEED" or "FAILED" send just once
/*  if (trajectory.has_parking_status()) {
    if (world_.parking_status() == "SUCCEED" || "FAILED"){
      world_.clear_parking_status();
    }
    else{
        world_.set_parking_status(ParkingStatus_Name(trajectory.parking_status()));
    }
    AINFO << "the ParkingStatus is: " << world_.parking_status();
  }*/
  ...
  if (world_.parking_status() == "SUCCEED"
  || world_.parking_status() == "FAILED"){
    if (preStatus_num == 4){
      hmi_worker_ -> ChangeToDrivingMode(Chassis::COMPLETE_MANUAL);
      AINFO << "backend to manual";
    }
  }
  preStatus = world_.parking_status();
  if (preStatus == "SUCCEED" || preStatus == "FAILED"){
    preStatus_num = preStatus_num + 1;
  }else {
    preStatus_num = 0;
  }
}
