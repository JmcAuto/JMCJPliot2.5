 ...
 * in the emulated world(仿真世界), including the car, the planning trajectory, etc.更新仿真世界中用到的数据
...
class SimulationWorldService {
 public:
   ...
   * @param routing_from_file whether to read initial routing from file. 从routing.proto中读取初始的routing数据
   */
  ...
  /**
   * @brief Get a read-only view of the SimulationWorld. world_是simulationWorld
   * 的常量引用
   ...
   * @return Json object equivalence（对等的、等价的） of the SimulationWorld object.
   */
 ...
 private:
  ...
  // 收到泊车状态后，退出自动驾驶
  const HMIWorker *hmi_worker_;
  // 上一帧泊车状态
  std::string preStatus = "";
