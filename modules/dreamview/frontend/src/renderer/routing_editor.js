...
export default class RoutingEditor {
    ...
    sendRoutingRequest(carOffsetPosition, coordinates) {
        ...
        WS.requestRoute(start, waypoint, end, parking_status);
        return true;
    }

    sendSummonRequest(carOffsetPosition, coordinates) {
        const parking_status = 0;
        if (this.routePoints.length === 0) {
            alert("Please provide an end point.");
            return false;
        }
        ...
        const end      = points[points.length-1];
        WS.requestRoute(start, waypoint, end, parking_status);
        WS.requestSummon(start, end);
        return true;
    }

    sendParkingRequest(carOffsetPosition, coordinates) {
        const start    = coordinates.applyOffset(carOffsetPosition, true);
        if (this.routePoints.length === 0) {
            alert("Please provide an end point or a parking space");
            return false;
        }
//        const confirm_ = confirm("Find parking space automatically!");
//        if (confirm_ === true) {
//            WS.requestParking_NOID(start);
//            return true;
//        }
        const points = this.routePoints.map((object) => {
            object.position.z = 0;
        ...
        const waypoint = [];
        const end      = points[points.length-1];
        WS.requestParking(start, end);
        return true;
    }
}

RoutingEditor.prototype.EDITING_MAP_RADIUS = 1500; //meters
