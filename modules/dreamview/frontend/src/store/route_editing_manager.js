...
export default class RouteEditingManager {
    ...
    sendSummonRequest(inNavigationMode) {
        if (!inNavigationMode) {
            const success = RENDERER.sendSummonRequest();
            if (success) {
                this.disableRouteEditing();
            }
            return success;
    ...
    sendParkingRequest(inNavigationMode) {
        if (!inNavigationMode) {
            const success = RENDERER.sendParkingRequest();
            if (success) {
                this.disableRouteEditing();
            }
            return success;
