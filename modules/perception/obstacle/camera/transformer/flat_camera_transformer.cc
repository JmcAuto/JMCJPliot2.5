...
bool FlatCameraTransformer::Transform(
  ...
  for (auto obj_ptr : *objects) {
    ...
    Eigen::Vector3f direction = obj_ptr->direction;
    Eigen::Vector4f center_v(center.x(), center.y(), center.z(), 0.0f);
    Eigen::Vector4f direction_v(direction.x(), direction.y(), direction.z(), 0.0f);
    //ADEBUG<<"ZZZ:"<<center_v.z();
    center_v = camera2car_ * center_v;
    direction_v = camera2car_ * direction_v;
    ...
    obj_ptr->center.x() = center_v.x() + camera2car_(0,3);
    obj_ptr->center.y() = center_v.y() + camera2car_(1,3);
    obj_ptr->center.z() = center_v.z() + camera2car_(2,3);
    //ADEBUG<<"XXX:"<<center_v.z()<<"YYY:"<<camera2car_(2,3)<<"X+Y:"<<obj_ptr->center.z();
    obj_ptr->direction = direction_v.head(3);

    obj_ptr->theta = atan2(direction_v[1], direction_v[0]);
