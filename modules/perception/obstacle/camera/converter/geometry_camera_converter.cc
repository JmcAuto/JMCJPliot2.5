...
bool GeometryCameraConverter::Convert(
  ...
  //20210408
  real_distance_x.clear();
  real_distance_y.clear();
  //20210408
  for (auto &obj : *objects) {
    ...
    if (obj->trunc_height < 0.25f) {
      ...
      ADEBUG << "object_id::" << obj-> id; 
      ConvertSingle(obj->height, obj->width, obj->length, obj->type, deg_alpha, upper_left,
    ...
    obj->center.x() = (world_distance_x_left + world_distance_x_right)/2-0.11;
    obj->center.y() = world_distance_y + 2.67;
    obj->center.z() = 0;
    obj->width = world_distance_x_right - world_distance_x_left;
    ADEBUG<<"obj_width:"<<obj->width;
...
bool GeometryCameraConverter::ConvertSingle(
    const float h, const float w, const float l, const ObjectType type, const float alpha_deg,
  ...
  //20210401 Find lowest line of object on image and caculate distance in real-world axis
  focal_length_x = camera_model_.get_focal_length_x();  //from camera intrinsic calibration
  focal_length_y = camera_model_.get_focal_length_y();  //from camera intrinsic calibration

  ground_point.x() = (upper_left.x()+lower_right.x())/2;
  ground_point.y() = lower_right.y();

  pixel_distance_x = ground_point.x() - 1920/2;
  pixel_distance_x_left = upper_left.x() - 1920/2;
  pixel_distance_x_right = lower_right.x() - 1920/2;

  pixel_distance_y = lower_right.y() - 1080/2;
  // pixel_distance_y = ground_point.y() - 1080/2;

  //ADEBUG<<"ground_point.x:"<<ground_point.x()<<"ground_point.y:"<<ground_point.y();
  gamma_angle = atan(pixel_distance_y /focal_length_y);
  //ADEBUG<<"gamma_angle:"<<gamma_angle<<"tan_gamma:"<<tan(gamma_angle);
  beta_angle = gamma_angle + camera_angle;
  //ADEBUG<<"tan_beta:"<<tan(beta_angle);
  world_distance_y = camera_height/tan(beta_angle);
  
  if(type ==  ObjectType::PEDESTRIAN || type ==  ObjectType::UNKNOWN || type ==  ObjectType::CAR){
    if(upper_left.y() < 471){
    world_distance_y = world_distance_y - 0.026*(471-upper_left.y());
    }
  }
  if(type ==  ObjectType::BUS || type ==  ObjectType::TRUCK){
    if(upper_left.y() < 90){
    world_distance_y = world_distance_y - 0.035*(90-upper_left.y());
    }
  }
  
  //world_distance_x = (camera_height*pixel_distance_x*focal_length/focal_length_x)/(sin(beta_angle)*pow((pow(pixel_distance_y*focal_length/focal_length_y,2)+pow(focal_length,2)),0.5));
  world_distance_x_left = (camera_height*pixel_distance_x_left*focal_length/focal_length_x)/(sin(beta_angle)*pow((pow(pixel_distance_y*focal_length/focal_length_y,2)+pow(focal_length,2)),0.5));
  world_distance_x_right = (camera_height*pixel_distance_x_right*focal_length/focal_length_x)/(sin(beta_angle)*pow((pow(pixel_distance_y*focal_length/focal_length_y,2)+pow(focal_length,2)),0.5));

  real_distance_y.push_back(world_distance_y);
  //real_distance_x.push_back(world_distance_x);
  real_distance_x.push_back(world_distance_x_right - world_distance_x_left);
  // real_distance_x.push_back((world_distance_x_right + world_distance_x_left)/2);
  //ADEBUG<< "real_distance_x:" << world_distance_x<< "real_distance_y:" << world_distance_y;
  //*distance = pow(pow(world_distance_x,2) + pow(world_distance_y,2),0.5);
  //20210401
...
void GeometryCameraConverter::CheckSizeSanity(
  ...
  if (obj->type == ObjectType::VEHICLE || obj->type == ObjectType::CAR || obj->type == ObjectType::BUS || obj->type == ObjectType::TRUCK) {
