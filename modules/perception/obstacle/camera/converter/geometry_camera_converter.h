...
class GeometryCameraConverter : public BaseCameraConverter {
 public:
  ...
  float get_real_distance_x(int i) override {return real_distance_x[i];}
  float get_real_distance_y(int i) override {return real_distance_y[i];}
  //20210408
  
  //20210414 Add return corners
  Eigen::Vector3f get_corners(int i) override {return corners_[i];}
  //20210414

 private:
  bool LoadCameraIntrinsics(const std::string &file_path);

  bool ConvertSingle(const float h, const float w, const float l, const ObjectType type, 
  ...
  const float camera_angle = 0.0;  //need camera extrinsic calibration
  const float camera_height = 1.46;  //need camera extrinsic calibration
  const float focal_length = 2.8;  //need camera intrinsic calibration
  float focal_length_x;  //from camera intrinsic calibration
  float focal_length_y;  //from camera intrinsic calibration

  float gamma_angle;
  float beta_angle;

  float pixel_distance_x;
  float pixel_distance_x_left;
  float pixel_distance_x_right;
  float pixel_distance_y;

  float world_distance_x;
  float world_distance_x_left;
  float world_distance_x_right;
  float world_distance_y;

  std::vector<float> real_distance_x;
  std::vector<float> real_distance_y;
