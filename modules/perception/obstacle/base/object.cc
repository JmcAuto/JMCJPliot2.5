...
using jmc_auto::common::util::Print;
...
void Object::Serialize(PerceptionObstacle* pb_obj) const {
  ...
  if(type == ObjectType::CAR || type == ObjectType::BUS || type == ObjectType::TRUCK){
    pb_obj->set_type(static_cast<PerceptionObstacle::Type>(ObjectType::VEHICLE));
  }
  else{
