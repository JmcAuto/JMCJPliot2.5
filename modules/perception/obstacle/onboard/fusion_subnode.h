...
class FusionSubnode : public Subnode {
 ...
 private:
  ...
  jmc_auto::common::Status Process(const EventMeta &event_meta,
                                 const std::vector<Event> &events);
  void RegistAllAlgorithm();

  void OnChassis(const jmc_auto::canbus::Chassis &message);

  void OnTteContiRadar(const jmc_auto::drivers::TteContiRadar &ttecontiradar);
  ...
  jmc_auto::drivers::TteContiRadar tte_chassis_;
  ...
  //TTE object data
  std::vector<std::shared_ptr<Object>> tte_objects_;
  std::vector<size_t> obj_type;
  std::vector<size_t> obj_x;
  std::vector<size_t> obj_y;
