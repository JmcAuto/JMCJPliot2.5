...
#ifndef MODULES_PERCEPTION_SLOT_CHECK_SLOTCHECK_PROCESS_SUBNODE_H_
#define MODULES_PERCEPTION_SLOT_CHECK_SLOTCHECK_PROCESS_SUBNODE_H_
...
class SlotCheckProcessSubnode : public Subnode 
{
 public:
 SlotCheckProcessSubnode() = default;
 ~SlotCheckProcessSubnode() = default;
...
//  bool FindRightSideSlot(const jmc_auto::perception::MulvalidSlot& mulvalidslot, unsigned int* choosen_index);
 bool GetSharedData(const Event &event, std::shared_ptr<SlotObjects> *slots);
 bool InitOutputStream();
 bool SubscribeEvents(const EventMeta &event_meta,
                      std::vector<Event> *event) const;

std::vector<std::pair<std::string, jmc_auto::perception::ValidMapSlot>> Delete_vector_pair_id(std::vector<std::pair<std::string, jmc_auto::perception::ValidMapSlot>> vector_pair,std::string delete_id);
...
//  std::unordered_map<std::string, jmc_auto::perception::ValidMapSlot> slots_to_check_;   //待检测障碍物的车位,std::unordered_map是无序
 std::vector<std::pair<std::string, jmc_auto::perception::ValidMapSlot>> slots_to_check_;
...
//  std::unordered_map<std::string, jmc_auto::perception::ValidMapSlot> slots_;//有效车位
 ...
 std::vector<jmc_auto::perception::PerceptionObstacles> perception_obstacles_vector_;
 ...
 long perception_obstacles_count_=0; //环视障碍物检测次数
...
//  bool has_obstacle_now_ = false;  //当前计算循环是否有障碍物
  ...
  SlotSharedData *slot_shared_data_ = nullptr;

  EventID slot_event_id_;

  std::shared_ptr<SlotObjects> roundview_slots_;
};

REGISTER_SUBNODE(SlotCheckProcessSubnode);
