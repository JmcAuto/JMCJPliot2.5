...
double Navigator::DistanceRouting(const RoutingRequest& request){
  if (!ShowRequestInfo(request, graph_.get())) {
    AERROR << "Error encountered when reading request point!";
    return -1;
  }
  ...
  if (!Init(request, graph_.get(), &way_nodes, &way_s)) {
    AERROR << "Failed to initialize navigator!";
    return -1;
  }
  ...
  if (result_nodes.empty()) {
    AERROR << "Failed to result nodes!";
    return request.waypoint().rbegin()->s() - request.waypoint().begin()->s();
  }
  result_nodes.front().SetStartS(request.waypoint().begin()->s());
  result_nodes.back().SetEndS(request.waypoint().rbegin()->s());
  return  result_generator_->CalDistance(result_nodes);
}
