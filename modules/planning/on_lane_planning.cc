...
using jmc_auto::common::EngageAdvice;
...
using jmc_auto::dreamview::Chart;
using jmc_auto::hdmap::HDMapUtil;
using jmc_auto::planning_internal::SLFrameDebug;
using jmc_auto::planning_internal::SpeedPlan;
using jmc_auto::planning_internal::STGraphDebug;
...
//  last_padmsg_.mutable_point()->mutable_end()->set_x(0);
...
Status OnLanePlanning::InitFrame(const uint32_t sequence_num,
  ...
  //NEW
//  auto ref = reference_lines.begin();
//  ADEBUG << "reference_lines.length:" << ref->Length();
//  if(reference_lines.get(0).empty()){
//    ADEBUG << "ref is empty";
//  }
  auto forward_limit =
      hdmap::PncMap::LookForwardDistance(vehicle_state.linear_velocity());
  ADEBUG << "LookForwardDistance : " << forward_limit;
...
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  //超声波安全距离
  // if(local_view_.tte_radar != nullptr){
  //   if(!need_referenceline){
  //     if(local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafrs_distance() * 100 < FLAGS_safedistance_normal ||
  //       local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafrm_distance() < FLAGS_safedistance_normal ||
  //       local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafr_distance() < FLAGS_safedistance_normal ||
  //       local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafls_distance() * 100 < FLAGS_safedistance_normal ||
  //       local_view_.tte_radar->debug_apafrontdistanceinfo_458().apaflm_distance() < FLAGS_safedistance_normal ||
  //       local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafl_distance() < FLAGS_safedistance_normal){
  //         ADCTrajectory estop_trajectory;
  //         EStop* estop = estop_trajectory.mutable_estop();
  //         estop->set_is_estop(true);
  //         estop->set_reason("the obstacle is within the safe distance");
  //         ADEBUG << "the obstacle is within the safe distance";
  ...
  //       }
  //   }else{
  //     if(local_view_.chassis->gear_location() == canbus::Chassis::GEAR_DRIVE){
  //       if(local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafrs_distance() * 100 < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafrm_distance() < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafr_distance() < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafls_distance() * 100< FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apafrontdistanceinfo_458().apaflm_distance() < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafl_distance() < FLAGS_safedistance_parking){
  //           ADCTrajectory estop_trajectory;
  //           EStop* estop = estop_trajectory.mutable_estop();
  //           estop->set_is_estop(true);
  //           ADEBUG << "the obstacle is within the safe distance";
  ...
  //           return;
  //       }
  //     }else if(local_view_.chassis->gear_location() == canbus::Chassis::GEAR_REVERSE){
  //       if(local_view_.tte_radar->debug_apareardistanceinfo_457().aparrs_distance() * 100 < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apareardistanceinfo_457().aparrm_distance() < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apareardistanceinfo_457().aparr_distance() < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apareardistanceinfo_457().aparls_distance() * 100 < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apareardistanceinfo_457().aparlm_distance() < FLAGS_safedistance_parking ||
  //         local_view_.tte_radar->debug_apareardistanceinfo_457().aparl_distance() < FLAGS_safedistance_parking){
  //           ADCTrajectory estop_trajectory;
  //           EStop* estop = estop_trajectory.mutable_estop();
  //           estop->set_is_estop(true);
  //           ADEBUG << "the obstacle is within the safe distance";
...
//  ADEBUG << "apa:" << local_view_.tte_radar->debug_apafrontdistanceinfo_458().apafrs_distance() * 100;
  bool newrouting = false;
  if (util::IsDifferentRouting(last_routing_, *local_view_.routing)) {
    last_routing_ = *local_view_.routing;
    ADEBUG << "routing is differrent, receive routing :" << last_routing_.DebugString();
    History::Instance()->Clear();
    PlanningContext::Instance()->mutable_planning_status()->Clear();
    reference_line_provider_->UpdateRoutingResponse(*local_view_.routing);
    planner_->Init(config_);
    //if(local_view_.pad_msg->appmode() != control::SUMMON_INPARKING){
    //    frame_->set_parking_destination(false);
    //  }
//    newrouting = true;
  }
...
//callback change current status
//  auto* previous_frame = FrameHistory::Instance()->Latest();
  if(local_view_.pad_msg->appmode() != planning::NORMAL && previous_frame != nullptr){
    if(previous_frame->pad_rerouting()){
      frame_->set_pad_rerouting(true);
    }
    if(previous_frame->parking_out()){
      frame_->set_parking_out(true);
    }
//    if(previous_frame->parking_start_collision()){
//      frame_->set_parking_start_collision(true);
//    }
    //frame_->set_parking_callback(previous_frame->parking_callback());
    //frame_->set_callback_rerouting(previous_frame->callback_rerouting());
    newrouting = previous_frame->rerouting_status();
    
    frame_->set_destination_replan(previous_frame->destination_replan());
    ADEBUG << "dest replan: destination replan : " << frame_->destination_replan();
  }
  ...
  if (!status.ok()) {
    ...
    if(need_referenceline){
      ptr_trajectory_pb->CopyFrom(previous_frame->current_frame_planned_trajectory());
      FillPlanningPb(start_timestamp, ptr_trajectory_pb);
      frame_->set_current_frame_planned_trajectory(*ptr_trajectory_pb);
      ADEBUG << "Not Referenceinfo so using last traj";
...
//callback change current status
////  auto* previous_frame = FrameHistory::Instance()->Latest();
//  if(local_view_.pad_msg->appmode() != control::NORMAL && previous_frame != nullptr){
//    if(previous_frame->pad_rerouting()){
//      frame_->set_pad_rerouting(true);
//    }
//    if(previous_frame->parking_out()){
...
//    newrouting = previous_frame->rerouting_status();
//  }
  //ADEBUG << "local_view_.pad_msg->point().end().x():" << local_view_.pad_msg->point().end().x() << " "<< local_view_.pad_msg->point().end().y();
  if(util::IsDifferentPadMessage(*local_view_.pad_msg, last_padmsg_)){
    last_padmsg_ = *local_view_.pad_msg;
    ...
    //  frame_->set_pad_rerouting(true);
     ...
     // }
   // }else
    if(frame_->parking_out() && local_view_.pad_msg->appmode() != planning::SUMMON){
      frame_->set_parking_out(false);
      if(frame_->is_parking_destination()){
        frame_->set_parking_destination(false);
      }
    }
//    if((local_view_.pad_msg->appmode() == control::PARKING || local_view_.pad_msg->appmode() == control::PARKING_NOID)
//        && frame_->parking_out()){
//        frame_->set_parking_out(false);
//    }
    if(local_view_.pad_msg->appmode() == planning::SUMMON_INPARKING){
       frame_->set_pad_rerouting(true);
       summoning_ = true;
    }
    ADEBUG << "new pad_msg";
  }
  if(frame_->pad_rerouting() && newrouting && !failed_to_update_reference_line){
    frame_->set_pad_rerouting(false);
    if(local_view_.pad_msg->appmode() == planning::SUMMON_INPARKING){
      frame_->set_parking_out(true);
      //frame_->set_parking_destination(false);
      ADEBUG << "SUMMON_INPARKING update complete";
    }
    frame_->set_parking_destination(false);
    ADEBUG << "rerouting update complete";
  }

//  if(previous_frame != nullptr){
//    frame_->set_parking_callback(previous_frame->parking_callback());
//    frame_->set_callback_rerouting(previous_frame->callback_rerouting());
//    newrouting = previous_frame->summon_reroutingsend();
//  }
//  ADEBUG << "local_view_.pad_msg->point().end().x():" << local_view_.pad_msg->point().end().x() << " "<< local_view_.pad_msg->point().end().y();
//  if(std::abs(local_view_.pad_msg->point().end().x() - last_padmsg_.point().end().x()) > 0.1 || std::abs(local_view_.pad_msg->point().end().y() - last_padmsg_.point().end().y()) > 0.1){
//    last_padmsg_ = *local_view_.pad_msg;
//    frame_->set_parking_callback(true);
//    ADEBUG << "start summon";
//  }
//  if(frame_->parking_callback() && newrouting && !failed_to_update_reference_line){
//    frame_->set_callback_rerouting(true);
//    frame_->set_parking_callback(false);
//    frame_->set_parking_destination(false);
//    ADEBUG << "Callback_rerouting update complete";
//  }
...
//NEW ADD
  ...
  status = Plan(start_timestamp, stitching_trajectory, ptr_trajectory_pb);
     ...
     if(vehicle_state.driving_mode() !=
        Chassis::DrivingMode::Chassis_DrivingMode_COMPLETE_AUTO_DRIVE){
          AERROR << "Not AUTO_DRIVE status, so stop";
          ptr_trajectory_pb->set_gear(canbus::Chassis::GEAR_DRIVE);
          FillPlanningPb(start_timestamp, ptr_trajectory_pb);
          GenerateStopTrajectory(ptr_trajectory_pb);
          frame_->set_current_frame_planned_trajectory(*ptr_trajectory_pb);
          const uint32_t num = frame_->SequenceNum();
          FrameHistory::Instance()->Add(num, std::move(frame_));
          return;
    }
  ...
  if (!status.ok()) {
    //解决泊出卡顿
//    if(local_view_.pad_msg->appmode() == control::SUMMON_INPARKING && frame_->parking_out()){
    if(summoning_ && previous_frame != nullptr){
      ptr_trajectory_pb->CopyFrom(previous_frame->current_frame_planned_trajectory());
      FillPlanningPb(start_timestamp, ptr_trajectory_pb);
      frame_->set_current_frame_planned_trajectory(*ptr_trajectory_pb);
  //    frame_->set_pad_rerouting(false);
...
//else if(summoning_ && !need_referenceline){
//      summoning_ = false;
//    }
//else if(local_view_.pad_msg->appmode() == control::SUMMON_INPARKING && !frame_->parking_out() && !need_referenceline){
//    frame_->set_parking_out(true);
//    ADEBUG << "Mode Change Success";
//  }
...
//    if(frame_->parking_status()){
//      ADEBUG << "PARKING SUCCED";
//    }
  if(previous_frame != nullptr && previous_frame->parking_status() != frame_->parking_status()){
    parking_status_send_count_ = 9;
    ptr_trajectory_pb->set_parking_status(frame_->parking_status());
    ADEBUG << "SEND STATUS:" << ptr_trajectory_pb->parking_status() << " " << frame_->parking_status();
  }else if(parking_status_send_count_ > 0){
    //frame_->set_parking_destination(false);
    ptr_trajectory_pb->set_parking_status(frame_->parking_status());
    ADEBUG << "short summon. SEND STATUS:" << ptr_trajectory_pb->parking_status() << " " << frame_->parking_status() << " count:" << parking_status_send_count_;
    parking_status_send_count_--;
  }
//else{
//    ptr_trajectory_pb->set_parking_status(0);
//  }
  const uint32_t n = frame_->SequenceNum();
  FrameHistory::Instance()->Add(n, std::move(frame_));
}
...
Status OnLanePlanning::Plan(
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) {
    ...
    if(frame_->borrow_parking()){
            ptr_trajectory_pb->mutable_decision()
          ->mutable_main_decision()
          ->mutable_parking()
          ->set_status(MainParking::ONE_PARKING);

        ptr_trajectory_pb->mutable_decision()
          ->mutable_main_decision()
          ->mutable_parking()
          ->set_parking_type(frame_->parking_type());
    }else{
      ptr_trajectory_pb->mutable_decision()
          ->mutable_main_decision()
          ->mutable_parking()
...
//    if(status != Status::OK() && local_view_.pad_msg->appmode() == control::SUMMON_INPARKING){
//      const uint32_t num = frame_->SequenceNum();
//      FrameHistory::Instance()->Add(num, std::move(frame_));
//    }
  } else {
    ...
    //if((best_ref_info->trajectory_type() != ADCTrajectory::NORMAL) && local_view_.pad_msg->appmode() == control::SUMMON_INPARKING){
    //  ADEBUG << "泊出等待正常轨迹生成";
    //  summoning_ = true;
   //` }

    //行车轨迹正常,泊出部分完成
    if(summoning_ && (best_ref_info->trajectory_type() == ADCTrajectory::NORMAL)){
      ADEBUG << "泊出部分完成";
      summoning_ = false;
    }
