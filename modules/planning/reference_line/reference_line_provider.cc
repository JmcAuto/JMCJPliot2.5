...
using jmc_auto::common::math::AngleDiff;
using jmc_auto::common::math::Vec2d;
using jmc_auto::common::time::Clock;
using jmc_auto::hdmap::HDMapUtil;
using jmc_auto::hdmap::LaneWaypoint;
using jmc_auto::hdmap::MapPathPoint;
...
ReferenceLineProvider::ReferenceLineProvider(
  ...
  ACHECK(jmc_auto::common::util::GetProtoFromFile(FLAGS_smoother_config_filename,
                                         &smoother_config_))
...
void ReferenceLineProvider::UpdateReferenceLine(
  ...
  // update history
  //解决行车突然停车问题,可能会有一点安全隐患
  if(reference_lines_.empty()){
    ADEBUG << "reference_lines_ is empty, so not update";
    return;
  }
...
bool ReferenceLineProvider::GetReferenceLinesFromRelativeMap(
  ...
  auto *adc_navigation_path = jmc_auto::common::util::FindOrNull(
  ...
  bool is_new_routing = false;
  {
    ...
    if (pnc_map_->IsNewRouting(routing)) {
     ...
     //is_reference_line_updated_ = true;
      //vehicle_state = common::VehicleStateProvider::instance()->vehicle_state();
    }
  }
  vehicle_state = common::VehicleStateProvider::instance()->vehicle_state();
