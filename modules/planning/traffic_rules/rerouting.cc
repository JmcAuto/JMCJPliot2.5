...
bool Rerouting::FindParkingRerouting(){
  auto* rerouting = PlanningContext::Instance()
                        ->mutable_planning_status()
                        ->mutable_rerouting();
  if (rerouting == nullptr) {
  ...
  if (!frame_->FindParkingRerouting()) {
    AERROR << "Failed to send summon rerouting request";
    return false;
  }
...
//callback
  if(frame_->find_parking_rerouting()){
    if(FindParkingRerouting()){
      ADEBUG << "callback_rerouting success";
//      frame->set_rerouting_status(true);
    }else{
      ADEBUG << "callback_rerouting fail ";
    }
    return Status::OK();
  }

  if(frame_->pad_rerouting()){
    if(PadRerouting()){
      ADEBUG << "callback_rerouting success";
      frame->set_rerouting_status(true);
    }else{
