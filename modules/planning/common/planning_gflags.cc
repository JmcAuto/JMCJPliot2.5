...
DEFINE_int32(planning_loop_rate, 20, "Loop rate for planning node");
...
DEFINE_double(look_forward_extend_distance, 20,
              "The step size when extending reference line.");
DEFINE_double(reference_line_stitch_overlap_distance, 10,
...
DEFINE_double(default_reference_line_width, 2.0,
...
DEFINE_double(lateral_jerk_bound, 12.0,
              "Bound of lateral jerk; symmetric for left and right");

DEFINE_double(kappa_bound, 0.3, "The bound for trajectory curvature");
...
// planning mode config file
DEFINE_string(planning_mode_config_file,
              "modules/planning/conf/planning_mode_config.pb.txt",
              "planning mode config file");
...
DEFINE_bool(enable_square_circle, false,
            "if true vehicle will go around square.");
...
DEFINE_double(safedistance_normal, 0.1,
              "(unit: meter/sec^2) for open space stand still at destination");

DEFINE_double(safedistance_parking, 0.2,
              "(unit: meter/sec^2) for open space stand still at destination");

DEFINE_bool(enable_parking_valid, true, "check parking vaild");

DEFINE_bool(enable_csv_debug, false, "print csv debug file.");

DEFINE_double(vertical_type_change_end_node_distance, 1.5,
              "vertical slot type change the end node y axis.");
