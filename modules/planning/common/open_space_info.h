...
struct GearSwitchStates {
  ...
  jmc_auto::canbus::Chassis::GearPosition gear_shift_position =
...
class OpenSpaceInfo {
 public:
  ...
  jmc_auto::planning_internal::Debug *mutable_debug() { return debug_; }

  void set_debug(jmc_auto::planning_internal::Debug *debug) { debug_ = debug; }

  const jmc_auto::planning_internal::Debug &debug() const { return *debug_; }

  const jmc_auto::planning_internal::Debug debug_instance() const {
    return debug_instance_;
  }

  jmc_auto::planning_internal::Debug *mutable_debug_instance() {
    return &debug_instance_;
  }
  ...
  void RecordDebug(jmc_auto::planning_internal::Debug *ptr_debug);

  void set_time_latency(double time_latency) { time_latency_ = time_latency; }

  bool need_truncate() const { return need_truncate_flag_; }

  void set_need_truncate(const bool flag) { need_truncate_flag_ = flag; }
 ...
 private:
  ...
  jmc_auto::planning_internal::Debug debug_instance_;

  double time_latency_ = 0.0;

  bool need_truncate_flag_ = false;
};
