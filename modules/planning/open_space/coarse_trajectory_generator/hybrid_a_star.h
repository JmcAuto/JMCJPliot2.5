 ...
 public:
  ...
  bool Plan(double sx, double sy, double sphi, double ex, double ey,
            ...
            const std::vector<std::vector<common::math::Vec2d>>&
                obstacles_vertices_vec, const bool gear,
            HybridAStartResult* result);
  bool TrajectoryPartition(const HybridAStartResult& result,
                           std::vector<HybridAStartResult>* partitioned_result);

  //cjx New add
  void SetNeedChangeEndNode(bool need_change_end_node) {need_change_end_node_ = need_change_end_node;}
//  bool IsStartNodeCollision(){return start_node_collision_;}

 private:
  bool AnalyticExpansion(std::shared_ptr<Node3d> current_node, const bool gear);
