...
ReedShepp::ReedShepp(const common::VehicleParam& vehicle_param,
  ...
  max_kappa_ = std::tan(vehicle_param_.max_steer_angle() /
               ...
               vehicle_param_.wheel_base() - 0.01;
...
bool ReedShepp::ShortestRSP(const std::shared_ptr<Node3d> start_node,
                            ...
                            std::shared_ptr<ReedSheppPath> optimal_path, const bool current_gear) {
...
//NEW ADD
//  std::sort(all_possible_paths.begin(), all_possible_paths.end(), 
//            [](const ReedSheppPath& a, const ReedSheppPath& b){return a.total_length < b.total_length;});
...
//    if (std::abs(all_possible_paths[i].x.back() -
//                end_node->GetX()) > 1e-3 ||
//        std::abs(all_possible_paths[i].y.back() -
//                 end_node->GetY()) > 1e-3 ||
//        std::abs(all_possible_paths[i].phi.back() -
//                 end_node->GetPhi()) > 1e-3) {
//      ADEBUG << "RSP end position not right nums: " << i;
//      continue;
//    }
//    if(all_possible_paths[i].gear.at(0) != current_gear){
...
//NEW ADD
  /*ADEBUG << "RS CHOICE IS:";
  for(auto& segs_type:shortest_path->segs_types){
     ADEBUG << segs_type;
  }*/
