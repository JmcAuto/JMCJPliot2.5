...
std::vector<std::shared_ptr<Node2d>> GridSearch::GenerateNextNodes(
  ...
  double step_size = 1;
  double diagonal_distance = std::sqrt(2.0) * step_size;
  std::vector<std::shared_ptr<Node2d>> next_nodes;
  std::shared_ptr<Node2d> up =
      std::make_shared<Node2d>(current_node_x, current_node_y + step_size, XYbounds_);
  up->SetPathCost(current_node_path_cost + step_size);
  std::shared_ptr<Node2d> up_right = std::make_shared<Node2d>(
      current_node_x + step_size, current_node_y + step_size, XYbounds_);
  up_right->SetPathCost(current_node_path_cost + diagonal_distance);
  std::shared_ptr<Node2d> right =
      std::make_shared<Node2d>(current_node_x + step_size, current_node_y, XYbounds_);
  right->SetPathCost(current_node_path_cost + step_size);
  ...
  std::shared_ptr<Node2d> down =
      std::make_shared<Node2d>(current_node_x, current_node_y - step_size, XYbounds_);
  down->SetPathCost(current_node_path_cost + step_size);
  ...
  std::shared_ptr<Node2d> left =
      std::make_shared<Node2d>(current_node_x - step_size, current_node_y, XYbounds_);
  left->SetPathCost(current_node_path_cost + step_size);
