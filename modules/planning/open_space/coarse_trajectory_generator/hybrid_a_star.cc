...
HybridAStar::HybridAStar(const PlannerOpenSpaceConfig& open_space_conf) {
  ...
  max_steer_angle_ =
      vehicle_param_.max_steer_angle() / vehicle_param_.steer_ratio() - 0.02;
...
bool HybridAStar::AnalyticExpansion(std::shared_ptr<Node3d> current_node, const bool gear) {
  ...
  //cjx 计算current_node到上边界的距离
  /*Box2d bounding_box = Node3d::GetBoundingBox(vehicle_param_, current_node->GetX(), current_node->GetY(), current_node->GetPhi());
  double min_dis0 = std::numeric_limits<double>::infinity();
  double min_dis1 = std::numeric_limits<double>::infinity();
  for(const auto& obstacle_linesegments : obstacles_linesegments_vec_){
    for(const common::math::LineSegment2d& linesegment : obstacle_linesegments){
      double dis0 = linesegment.DistanceTo(bounding_box.GetAllCorners().at(0));
      double dis1 = linesegment.DistanceTo(bounding_box.GetAllCorners().at(1));
  ...
  ADEBUG << "pre_final node x:" << current_node->GetX() << " y:" << current_node->GetY() << " phi:" << current_node->GetPhi();
  //ADEBUG << "min dis0 = " << min_dis0 << " min dis1 = " << min_dis1;
  return true;
}
...
bool HybridAStar::ValidityCheck(std::shared_ptr<Node3d> node) {
  ...
  for (size_t i = check_start_index; i < node_step_size; ++i) {
    ...
    //ADEBUG << std::setprecision(9) << "ego collision: bounding_box point0 { " << bounding_box.GetAllCorners()[0].x() << "," << bounding_box.GetAllCorners()[0].y() << " }";
    //ADEBUG << std::setprecision(9) << "ego collision: bounding_box point1 { " << bounding_box.GetAllCorners()[1].x() << "," << bounding_box.GetAllCorners()[1].y() << " }";
    //ADEBUG << std::setprecision(9) << "ego collision: bounding_box point2 { " << bounding_box.GetAllCorners()[2].x() << "," << bounding_box.GetAllCorners()[2].y() << " }";
    //ADEBUG << std::setprecision(9) << "ego collision: bounding_box point3 { " << bounding_box.GetAllCorners()[3].x() << "," << bounding_box.GetAllCorners()[3].y() << " }";
    for (const auto& obstacle_linesegments : obstacles_linesegments_vec_) {
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
    std::shared_ptr<Node3d> current_node, size_t next_node_index, const bool gear) {
  double steering = 0.0;
...
bool HybridAStar::GetResult(HybridAStartResult* result) {
  ...
  //cjx New add 移动end node后补全轨迹
  if(need_change_end_node_){
    double last_x = hybrid_a_x.back();
    double last_y = hybrid_a_y.back();
    Vec2d last_point(last_x, last_y);
    Vec2d last_last_point(hybrid_a_x.at(hybrid_a_x.size() - 2), hybrid_a_y.at(hybrid_a_y.size() -2));
    double last_point_dis = last_point.DistanceTo(last_last_point);
    double last_phi = hybrid_a_phi.back();
    double traveled_distance = step_size_;
    ADEBUG << "change end node trajectory stitching.. last x:" << last_x << " last y:" << last_y << " last phi:" << last_phi;
    
    size_t points_count = 0;
    double remain_distance = FLAGS_vertical_type_change_end_node_distance;
    if((traveled_distance > last_point_dis) && (last_point_dis > 1e-4)){
      double last_point_append_dis = traveled_distance - last_point_dis;
      remain_distance = FLAGS_vertical_type_change_end_node_distance - last_point_append_dis;
      
      const double next_x = last_x - last_point_append_dis * std::cos(last_phi);
      const double next_y = last_y - last_point_append_dis * std::sin(last_phi);

      //pop back
      hybrid_a_x.pop_back();
      hybrid_a_y.pop_back();
      hybrid_a_phi.pop_back();

      hybrid_a_x.push_back(next_x);
      hybrid_a_y.push_back(next_y);
      hybrid_a_phi.push_back(last_phi);
    ...
    size_t traj_nums = remain_distance / traveled_distance;
    for(size_t i = 0; i < traj_nums; ++i){
      const double next_x = last_x - traveled_distance * std::cos(last_phi);
      const double next_y = last_y - traveled_distance * std::sin(last_phi);
      hybrid_a_x.push_back(next_x);
      hybrid_a_y.push_back(next_y);
    ...
    remain_distance -= traveled_distance * traj_nums;

    if(remain_distance > 1e-4){
      hybrid_a_x.push_back(last_x - remain_distance * std::cos(last_phi));
      hybrid_a_y.push_back(last_y - remain_distance * std::sin(last_phi));
      hybrid_a_phi.push_back(last_phi);

      ++points_count;
    }
    ADEBUG << std::setprecision(9) << "change end node end x= " << hybrid_a_x.back() << " y= " << hybrid_a_y.back() << " phi= " << hybrid_a_phi.back() << " add " << points_count << " points.";
  }
...
bool HybridAStar::Plan(
    ...
    const std::vector<std::vector<common::math::Vec2d>>& obstacles_vertices_vec, const bool gear,
  ...
  //cjx New add 垂直车位移动end node的y坐标
  if(need_change_end_node_){
    if(ephi > 0){
      end_node_.reset(
          new Node3d({ex}, {ey + FLAGS_vertical_type_change_end_node_distance}, {ephi}, XYbounds_, planner_open_space_config_));
      ADEBUG << "change end node after: ex= " << ex << " ey= " << ey << " + " << FLAGS_vertical_type_change_end_node_distance << " = " << ey + FLAGS_vertical_type_change_end_node_distance << " ephi= " << ephi;
    }else{
      end_node_.reset(
          new Node3d({ex}, {ey - FLAGS_vertical_type_change_end_node_distance}, {ephi}, XYbounds_, planner_open_space_config_));
      ADEBUG << "change end node after: ex= " << ex << " ey= " << ey << " - " << FLAGS_vertical_type_change_end_node_distance << " = " << ey - FLAGS_vertical_type_change_end_node_distance << " ephi= " << ephi;
    ...
    ADEBUG << "change end node no: ex= " << ex << " ey= " << ey << " ephi= " << ephi;
  }
...
//    start_node_collision_ = true;
  ...
  while (!open_pq_.empty()) {
    ...
    if (AnalyticExpansion(current_node, gear)) {
      break;
    }
   ...
   // if(the_first_){
   //   std::shared_ptr<Node3d> next_node = std::shared_ptr<Node3d>(
   //   new Node3d(2.279063, 1.01140, 1.1, XYbounds_,
  //               planner_open_space_config_));
  //    next_node->SetPre(current_node);
  //    next_node->SetDirec(false);
      //next_node->SetSteer(-300);
 ...
 //     continue;
  //  }
    ...
    for (size_t i = 0; i < next_node_num_; ++i) {
      std::shared_ptr<Node3d> next_node = Next_node_generator(current_node, i, gear);
