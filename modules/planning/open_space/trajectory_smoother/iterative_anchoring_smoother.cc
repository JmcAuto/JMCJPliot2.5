...
using jmc_auto::common::math::LineSegment2d;
...
bool IterativeAnchoringSmoother::Smooth(
  ...
  //cjx Newadd enfore rear straight
  enforce_rear_straight_count_ = 0;
  auto last_point_phi = interpolated_warm_start_path.back().theta();
  for(auto it = interpolated_warm_start_path.rbegin(); it != interpolated_warm_start_path.rend(); ++it){
    if(std::fabs(it->theta() - last_point_phi) < 1e-4){
      ++enforce_rear_straight_count_;
    }else{
      break;
    }
  }
  ADEBUG << "change end node : enforce rear straight count : " << enforce_rear_straight_count_;
...
void IterativeAnchoringSmoother::AdjustPathBounds(
  ...
  //cjx Newadd enforce rear straight
  if(enforce_rear_straight_count_ > 6){
    ADEBUG << "change end node : " << enforce_rear_straight_count_ * 0.8 << " points will be enforced.";
    for(size_t i = 1; i < enforce_rear_straight_count_ * 0.8; ++i){
      bounds->at(bounds->size() - i) = 0.0;
    }
    enforce_rear_straight_count_ = 0;
  }
}
