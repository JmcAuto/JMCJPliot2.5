...
using jmc_auto::hdmap::LaneSegment;
...
OpenSpaceRoiDecider::OpenSpaceRoiDecider(const TaskConfig &config)
  ...
  vehicle_params_ =
      jmc_auto::common::VehicleConfigHelper::GetConfig().vehicle_param();
}
...
Status OpenSpaceRoiDecider::Process(Frame *frame) {
  ...
  //START ADD
//  start_collision_ = frame->parking_start_collision();
...
//      if(summon_direction_ == UNKNOW){
//        ADEBUG << "JISUAN";
        SummonDirection(frame, spot_vertices);
        ADEBUG << "SUMMON TYPE:" << summon_direction_;
//      }
    }else{
      summon_direction_ = UNKNOW;
    }

    SetOrigin(frame, spot_vertices);

    CheckParkingType(frame, spot_vertices);

    CheckNeedTruncate(frame);
...
//NEW ADD  HORIZONTAL PARKING
void OpenSpaceRoiDecider::CheckParkingType(Frame *const frame, const std::array<common::math::Vec2d, 4> &vertices){
    LaneInfoConstPtr nearest_lane;
    common::PointENU point;
    point.set_x(frame->local_view().pad_msg->parking_space_info().point_left_up().x());
    point.set_y(frame->local_view().pad_msg->parking_space_info().point_left_up().y());
    double s = 0.0;
    double l = 0.0;
    int status = hdmap_->GetNearestLane(point, &nearest_lane, &s, &l);
    if(status != 0){
      ADEBUG << "Find Nearest Lane Failed, So Check parking type faild";
      return;
    }
    double lane_heading = nearest_lane->Heading(s);
    double parking_heading = (vertices[0] - vertices[1]).Angle();
    double heading_diff = std::abs(common::math::NormalizeAngle(lane_heading - parking_heading));
    ADEBUG << "lane head:" << lane_heading << " parking head:" << parking_heading << " diff:" << heading_diff;
    if(heading_diff < 0.2){
      ADEBUG << "HORIZONTAL";
      parking_type_ = HORIZONTAL;
      frame->set_parking_type(planning::Parking_Type::HORIZONTAL);
    }else if(heading_diff > 1.2){
      parking_type_ = VERTICAL;
      frame->set_parking_type(planning::Parking_Type::VERTICAL);
    }else{
      parking_type_ = OBLIQUE;
      frame->set_parking_type(planning::Parking_Type::OBLIQUE);
    }
}

void OpenSpaceRoiDecider::CheckNeedTruncate(Frame *const frame){
    if(frame->local_view().pad_msg->appmode() != planning::SUMMON_INPARKING){
      return;
    }

    jmc_auto::common::math::Vec2d summon_point;
    auto end_x_enu = frame->local_view().pad_msg->point().end().x();
    auto end_y_enu = frame->local_view().pad_msg->point().end().y();
    summon_point.set_x(end_x_enu);
    summon_point.set_y(end_y_enu);

    const auto &origin_point = frame->open_space_info().origin_point();
    const auto &origin_heading = frame->open_space_info().origin_heading();

    summon_point -= origin_point;
    summon_point.SelfRotate(-origin_heading);

    jmc_auto::common::PointENU start_point;
    start_point.set_x(frame->local_view().pad_msg->parking_space_info().point_left_up().x());
    start_point.set_y(frame->local_view().pad_msg->parking_space_info().point_left_up().y());

    double s = 0.0;
    double l = 0.0;
    LaneInfoConstPtr nearest_lane;
    int status = hdmap_->GetNearestLane(start_point, &nearest_lane, &s, &l);
    if(status != 0){
      ADEBUG << "short summon. failed to find nearest lane.";
      return;
    }

    jmc_auto::common::math::Box2d judge_box;
    
    if(parking_type_ == HORIZONTAL){
    
      auto box_width = nearest_lane->GetWidth(s);
      judge_box = jmc_auto::common::math::Box2d::CreateAABox({0.0, -0.3}, {-box_width, 5.0});
    
    }else if((parking_type_ == VERTICAL) || ( parking_type_ == OBLIQUE)){
      
      if(summon_direction_ == RIGHT_SUMMON){

        auto box_width = nearest_lane->GetWidth(s);
        judge_box = jmc_auto::common::math::Box2d::CreateAABox({0.0, 0.0}, {10.0, box_width});
      
      }else if(summon_direction_ == LEFT_SUMMON){
      
        auto box_width = nearest_lane->GetRoadWidth(s);
        judge_box = jmc_auto::common::math::Box2d::CreateAABox({2.7, 0.0}, {-10.0, box_width});
      
      }else{
        ADEBUG << "short summon. unknown summon direction.";
        return;
      }
    
    }else{
      ADEBUG << "short summon. unknown parking type.";
      return;
    }

    auto is_point_in = judge_box.IsPointIn(summon_point);
    ADEBUG << "short summon. parking type : " << parking_type_ << " summon direction : " << summon_direction_ << " is point in : " << is_point_in;

    if(is_point_in){
      frame->mutable_open_space_info()->set_need_truncate(true);
    }

}

void OpenSpaceRoiDecider::SummonDirection(Frame *const frame, const std::array<common::math::Vec2d, 4> &vertices){
    auto left_top = vertices[0];
    auto right_top = vertices[3];
    common::PointENU rightsummon_point;
    rightsummon_point.set_x((left_top.x() + right_top.x())/2);
    rightsummon_point.set_y((left_top.y() + right_top.y())/2);

    //左边没空间
    double rightsummon_s = 0.0;
    double rightsummon_l = 0.0;
    hdmap::LaneInfoConstPtr rightsummon_lane;
    Vec2d heading_vec = vertices[0] - vertices[1];
    if (hdmap_->GetNearestLaneWithHeading(
            rightsummon_point, 10.0, heading_vec.Angle(), M_PI / 1.5, &rightsummon_lane,
            &rightsummon_s, &rightsummon_l) == 0) {
    //  std::cout << "111";
      if(rightsummon_lane->lane().turn() != 1){
        ADEBUG << "parking in the turn lane" << rightsummon_lane->lane().id().id();
        summon_direction_ = rightsummon_s < 3 ? LEFT_SUMMON : RIGHT_SUMMON;
        return;
      }
      if(rightsummon_s < 10){
        int pre_lanes_num = rightsummon_lane->lane().predecessor_id_size();
        if(pre_lanes_num != 0){
          bool no_turn = false;
          for(int i = 0; i < pre_lanes_num; ++i){
            auto pre_lane_id = rightsummon_lane->lane().predecessor_id(i);
            auto pre_lane = hdmap_->GetLaneById(pre_lane_id);
            if(pre_lane->lane().turn() == 1){
              no_turn = true;
              break;
            }
          }
          if(!no_turn){
            ADEBUG << "left not summon space choice right summon";
            summon_direction_ = RIGHT_SUMMON;
            return;
          }
        }
      }
    }else{
      ADEBUG << "summon get nearestlane failed";
      return;
    }
    //右边没空间
   double leftsummon_s = 0.0;
    hdmap::LaneInfoConstPtr leftsummon_lane;
    if(rightsummon_lane->lane().left_neighbor_reverse_lane_id_size() > 0){
      auto left_laneid = rightsummon_lane->lane().left_neighbor_reverse_lane_id(0);
      leftsummon_lane = hdmap_->GetLaneById(left_laneid);
      leftsummon_s = leftsummon_lane->total_length() - rightsummon_s;
    }else{
      common::PointENU leftsummon_point;
      double leftsummon_l = 0.0;
      leftsummon_point.set_x((left_top.x() + right_top.x())/2);
      leftsummon_point.set_y(((left_top.y() + right_top.y())/2) + 4.5);
      int status = hdmap_->GetNearestLane(leftsummon_point, &leftsummon_lane, &leftsummon_s, &leftsummon_l);
      if(status != 0){
        ADEBUG << "left summon get nearestlane failed";
        return;
      }
//      if(leftsummon_lane->lane().turn() != 1){
//        ADEBUG << "parking in the turn lane: " << leftsummon_lane->lane().id().id();
//        summon_direction_ = leftsummon_s < 3 ? RIGHT_SUMMON : LEFT_SUMMON;
//        return;
//      }
    }
    if(leftsummon_s < 12){
        int next_lanes_num = leftsummon_lane->lane().predecessor_id_size();
        if(next_lanes_num != 0){
          bool No_turn = false;
          for(int i = 0; i < next_lanes_num; ++i){
            auto next_lane_id = leftsummon_lane->lane().predecessor_id(i);
            auto next_lane = hdmap_->GetLaneById(next_lane_id);
            if(next_lane->lane().turn() == 1){
              No_turn = true;
              break;
            }
          }
          if(!No_turn){
            ADEBUG << "right not summon space choice left summon";
            summon_direction_ = LEFT_SUMMON;
            return;
          }
        }
    }
...
//    auto left_laneid = rightsummon_lane->lane().left_neighbor_reverse_lane_id(0);
//    auto leftsummon_lane = hdmap_->GetLaneById(left_laneid);
//    double leftsummon_s = leftsummon_lane->total_length() - rightsummon_s;
//    if(leftsummon_s < 12){
//        int next_lanes_num = rightsummon_lane->lane().successor_id_size();
//        if(next_lanes_num != 0){
//          bool No_turn = false;
//          for(int i = 0; i < next_lanes_num; ++i){
//            auto next_lane_id = rightsummon_lane->lane().successor_id(i);
//            auto next_lane = hdmap_->GetLaneById(next_lane_id);
//            if(next_lane->lane().turn() == 1){
//              No_turn = true;
//              break;
...
//    }
    ...
    summon_point.set_x(frame->local_view().pad_msg->point().end().x());
    summon_point.set_y(frame->local_view().pad_msg->point().end().y());
    double summon_s = 0.0;
    double summon_l = 0.0;
    ...
    //左泊出路径构建
    auto request_left = frame->local_view().routing->routing_request();
    request_left.clear_header();
    request_left.clear_waypoint();
    auto *left_point = request_left.add_waypoint();
    left_point->set_id(leftsummon_lane->id().id());
    left_point->set_s(leftsummon_s);
    left_point->mutable_pose()->CopyFrom(rightsummon_point);
    auto *end_point1 = request_left.add_waypoint();
    end_point1->set_id(summon_lane->id().id());
    end_point1->set_s(summon_s);
    end_point1->mutable_pose()->CopyFrom(summon_point);
    //右泊出路径构建
    auto request_right = frame->local_view().routing->routing_request();
    request_right.clear_header();
    request_right.clear_waypoint();
    auto *right_point = request_right.add_waypoint();
    right_point->set_id(rightsummon_lane->id().id());
    right_point->set_s(rightsummon_s);
    right_point->mutable_pose()->CopyFrom(rightsummon_point);
    auto *end_point2 = request_right.add_waypoint();
    end_point2->set_id(summon_lane->id().id());
    end_point2->set_s(summon_s);
    end_point2->mutable_pose()->CopyFrom(summon_point);
    //获取路径长度
    routing::Navigator navigator(jmc_auto::hdmap::RoutingMapFile());
//    navigator_ptr.reset(new routing::Navigator(jmc_auto::hdmap::RoutingMapFile()));
//    ADEBUG << jmc_auto::hdmap::RoutingMapFile();
    //double left_distance = navigator_ptr->DistanceRouting(request_left);
    double right_distance = navigator.DistanceRouting(request_right);
    double left_distance = navigator.DistanceRouting(request_left);
    if(left_distance < 0 && right_distance < 0) return;
    //选择短的路径泊出
    ADEBUG << "left_summon_distance:" << left_distance << " right_summon_distance:" << right_distance;
    if(left_distance > right_distance && right_distance > 0){
      summon_direction_ = RIGHT_SUMMON;
    }else{
      summon_direction_ = LEFT_SUMMON;
    }
...
//NEW ADD callback
  if(frame->local_view().pad_msg->appmode() == 4){
    //NEW ADD HORIZONTAL PARKING
    if(parking_type_ == HORIZONTAL){
      ADEBUG << "HORIZONTAL parking endpose";
      end_x = -2;
      end_y = 5;
    ...
    ADEBUG << "summon endpose";
  //  auto xy_boundary = frame->open_space_info().ROI_xy_boundary();
  //  if (end_x < xy_boundary[0] || end_x > xy_boundary[1] ||
  //    end_y < xy_boundary[2] || end_y > xy_boundary[3]) {
  //    AERROR << "end_pose of xy boundary of parking ROI, So not use second parking";
  //    frame->set_second_parking(false);
  //  }else{
      end_pose->push_back(end_x);
      end_pose->push_back(end_y);
      end_pose->push_back(summon_direction_ == RIGHT_SUMMON ? common::math::NormalizeAngle(parking_spot_heading + M_PI_2) : common::math::NormalizeAngle(parking_spot_heading - M_PI_2));
      //end_pose->push_back(
     //   common::math::NormalizeAngle(parking_spot_heading + M_PI));
      end_pose->push_back(0.0);
  //    ADEBUG << " First heading:" << end_pose->at(2);
  //    ADEBUG << "parking_spot_heading:" << parking_spot_heading;
      return;
  //  }
  }
...
//NEW ADD
       ...
       end_y = left_down.y() - vehicle_params_.back_edge_to_center() - parking_depth_buffer;
...
//NEW ADD
   ...
   //                        parking_depth_buffer);
      ...
      end_y = left_down.y() + vehicle_params_.back_edge_to_center() + parking_depth_buffer;
     // if(left_down.y() > -5.6) end_y = left_down.y() + 0.5;
//      if(end_y < -4.2) end_y = -4.5;
//NEW ADD
...
//                               parking_depth_buffer);
 ...
 //     frame->mutable_open_space_info()->mutable_open_space_end_pose();
//适配倾斜车位
  if(parking_type_ == OBLIQUE){
    double buff = 0.2;
    double off_x = std::cos(common::math::NormalizeAngle(parking_spot_heading + M_PI))*1.2;
    double off_y = std::sin(common::math::NormalizeAngle(parking_spot_heading + M_PI))*1.2;
    end_x = (left_down.x() + right_down.x()) / 2.0 + off_x + buff;
    end_y = (left_down.y() + right_down.y()) / 2.0 + off_y + buff;
    ADEBUG << "OFF X:" << off_x << " y:" << off_y;
  }
...
void OpenSpaceRoiDecider::GetRoadBoundary(
    Frame *const frame, const hdmap::Path &nearby_path, const double center_line_s,
 ...
 // }
  ...
  if(summon_direction_ == RIGHT_SUMMON || parking_type_ == HORIZONTAL){
    start_s = center_line_s - 5;
    end_s = center_line_s + 15;
  }

//NEW ADD  解决寻找车位模式不能泊入问题
  if(frame->local_view().pad_msg->appmode() == 2){
//    common::math::Vec2d vehicle_point(vehicle_state_.x(), vehicle_state_.y());
//    double vehicle_s = 0.0;
//    double vehicle_l = 0.0;
//    ADEBUG << "NOID mode end_s adjust";
//    if (!(nearby_path.GetProjection(vehicle_point, &vehicle_s, &vehicle_l))){
//      AERROR << "fail to get parking spot points' projections on reference line";
//    }else{
    //  ADEBUG << "NOIDIS:" << center_line_s;
      end_s = center_line_s + 12;
//    }
  }


  ADEBUG << "test core20220719 : start_s = " << start_s;
  for(const auto lane_seg : nearby_path.lane_segments()){
    ADEBUG << "test core20220719 lane id : " << lane_seg.lane->id().id() << " start s : " << lane_seg.start_s << " end s : " << lane_seg.end_s;
  }
  ...
  while (check_point_s <= end_s) {
    ...
    ADEBUG << "is is_center_lane_heading_change: "
           << is_center_lane_heading_change << " check_point_s = " << check_point_s;
...
// ||                           is_center_lane_heading_change;
    ...
    // NEW ADD ONE PARKING
    bool is_anchor_point = frame->borrow_parking() ? (check_point_s == start_s || check_point_s == end_s ||
                                                     is_center_lane_heading_change) : 
                                                     (check_point_s == start_s || check_point_s == end_s);

    // Add key points to the left-half boundary
    AddBoundaryKeyPoint(frame, nearby_path, check_point_s, start_s, end_s,
  ...
  //  ADEBUG << "LEFT_WIDTH:" << left_lane_road_width->at(i);
    left_lane_boundary->at(i) -= origin_point;
    left_lane_boundary->at(i).SelfRotate(-origin_heading);
//    ADEBUG << "left bound x:" << left_lane_boundary->at(i).x() << " y:" << left_lane_boundary->at(i).y();
  }
  for (size_t i = 0; i < right_point_size; i++) {
//    ADEBUG << "RIGHT_WIDTH:" << right_lane_road_width->at(i);
    right_lane_boundary->at(i) -= origin_point;
    right_lane_boundary->at(i).SelfRotate(-origin_heading);
//    ADEBUG << "right bound x:" << right_lane_boundary->at(i).x() << " y:" << right_lane_boundary->at(i).y();
  }
}
...
void OpenSpaceRoiDecider::AddBoundaryKeyPoint(
    Frame *const frame, const hdmap::Path &nearby_path, const double check_point_s,
  ...
  if(CheckIsinJunctionByS(nearby_path, check_point_s)){
     current_road_width =
      is_left_curb ? nearby_path.GetRoadLeftWidth(check_point_s)*2 +
  ...
  //START ADD
//  if(is_left_curb && start_collision_){
//    current_road_width += 0.3;
//  }
...
//NEW ADD
  ...
  if(CheckIsinJunctionByS(nearby_path, (check_point_s - previous_distance_s))){
    previous_road_width =
      is_left_curb ? nearby_path.GetRoadLeftWidth(check_point_s - previous_distance_s)*2 +
  ...
  if(CheckIsinJunctionByS(nearby_path, (check_point_s + next_distance_s))){
    next_road_width =
      is_left_curb ? nearby_path.GetRoadLeftWidth(check_point_s + next_distance_s)*2 +
                     nearby_path.GetRoadRightWidth(check_point_s + next_distance_s)
                   : nearby_path.GetRoadRightWidth(check_point_s + next_distance_s);
  }else{
    next_road_width =
      is_left_curb ? nearby_path.GetRoadLeftWidth(check_point_s + next_distance_s)
                   : nearby_path.GetRoadRightWidth(check_point_s + next_distance_s);
  }
...
//          : nearby_path.GetRoadRightWidth(check_point_s + next_distance_s);
  ...
  // as a key point.
  if (!frame->borrow_parking() || std::abs(current_curb_point_delta_theta) >
...
//  auto left_down = vertices[2];
//  auto right_down = vertices[3];
//  auto right_top = vertices[0];
...
//NEW ADD  HORIZONTAL PARKING
  if(parking_type_ == HORIZONTAL){
    left_top = vertices[1];
    left_down = vertices[2];
    right_down = vertices[3];
    right_top = vertices[0];
  }else{
    left_top = vertices[0];
  ...
  const auto &origin_point = frame->open_space_info().origin_point();
  ADEBUG << std::setprecision(9) << "origin_point: " << origin_point.x() << ", " << origin_point.y();
  const auto &origin_heading = frame->open_space_info().origin_heading();

  //NEW ADD 解决泊车压角问题
 // if(left_top.x() - right_top.x() <=0){
//    right_down.set_x(right_down.x() - 0.2);
//    right_top.set_x(right_down.x() - 0.2);
//  }else{
//    right_down.set_x(right_down.x() + 0.2);
//    right_top.set_x(right_down.x() + 0.2);
//  }
  ...
  //left_top.set_x(left_top.x() - 2);
  //left_down.set_x(left_down.x() - 2);

//解决压线不能泊出问题
  if(parking_type_ != HORIZONTAL && (frame->local_view().pad_msg->appmode() == 4 || frame->borrow_parking() || frame->destination_replan())){
//    left_top.set_x(left_top.x() - 0.3);
//    left_down.set_x(left_down.x() - 0.3);
    double buff = 1.5;
    double offset = std::fabs(vehicle_state_.x() - origin_point.x());
    ADEBUG << std::setprecision(9) << "ego collision: offset = " << vehicle_state_.x() << " - " << origin_point.x() << " = " << offset;
    if(offset <= 1){
      left_top.set_x(left_top.x() + offset - buff);
      left_down.set_x(left_down.x() + offset - buff);
      ADEBUG << "ego collision: left side collides. <<<";
    }else if(offset >= 1.6){
      right_top.set_x(right_top.x() + offset + buff);
      right_down.set_x(right_down.x() + offset + buff);
      ADEBUG << "ego collision: right side collides. >>>";
    }
    else{
      double ego_length = vehicle_params_.length();
      double ego_width = vehicle_params_.width();
      double shift_distance =
           ego_length / 2.0 - vehicle_params_.back_edge_to_center();
      
      common::math::Vec2d vehicle_state_ref(vehicle_state_.x(), vehicle_state_.y());
      vehicle_state_ref -= origin_point;
      vehicle_state_ref.SelfRotate(-origin_heading); 
      double phi_ref = common::math::NormalizeAngle(vehicle_state_.heading() - origin_heading);
      Box2d ego_box(
         {vehicle_state_ref.x() + shift_distance * std::cos(phi_ref), vehicle_state_ref.y() + shift_distance * std::sin(phi_ref)}, phi_ref, ego_length, ego_width);
      ADEBUG << std::setprecision(9) << "ego collision: ego_center { " << ego_box.center_x() << "," << ego_box.center_y() << " }";
      ADEBUG << std::setprecision(9) << "ego collision: ego heading : " << ego_box.heading();
      ADEBUG << std::setprecision(9) << "ego collision: shift distance : " << shift_distance;
      ADEBUG << std::setprecision(9) << "ego collision: ego_box point0 { " << ego_box.GetAllCorners()[0].x() << "," << ego_box.GetAllCorners()[0].y() << " }"; 
      ADEBUG << std::setprecision(9) << "ego collision: ego_box point1 { " << ego_box.GetAllCorners()[1].x() << "," << ego_box.GetAllCorners()[1].y() << " }"; 
      ADEBUG << std::setprecision(9) << "ego collision: ego_box point2 { " << ego_box.GetAllCorners()[2].x() << "," << ego_box.GetAllCorners()[2].y() << " }"; 
      ADEBUG << std::setprecision(9) << "ego collision: ego_box point3 { " << ego_box.GetAllCorners()[3].x() << "," << ego_box.GetAllCorners()[3].y() << " }"; 
      ADEBUG << std::setprecision(9) << "ego collision: left top { " << left_top.x() << "," << left_top.y() << " }";
      ADEBUG << std::setprecision(9) << "ego collision: left down { " << left_down.x() << "," << left_down.y() << " }";
     // common::math::Vec2d ego_position(vehicle_state_.x(), vehicle_state_.y());
     // Box2d ego_box(ego_position, vehicle_state_.heading(), ego_length, ego_width);
      //偏左压角
      common::math::LineSegment2d left_bound(left_top, left_down);
      if(ego_box.HasOverlap(left_bound)){
        ADEBUG << std::setprecision(9) << "ego collision: left top { " << left_top.x() << "," << left_top.y() << " }";
        ADEBUG << std::setprecision(9) << "ego collision: left down { " << left_down.x() << "," << left_down.y() << " }";
        ADEBUG << "ego collision: left side corner collides with the parking line. <<<<<";
        double off_set = std::min(std::fabs(left_top.x() - ego_box.max_x()), std::fabs(left_top.x() - ego_box.min_x()));
        left_top.set_x(left_top.x() - off_set - 0.5);
        left_down.set_x(left_down.x() - off_set - 0.5);
      }
      else{
        //偏右压角
        common::math::LineSegment2d right_bound(right_top, right_down);
        //common::math::Vec2d right_top_point(right_top.x(), right_top.y());
        //right_top_point.SelfRotate(origin_heading);
        //right_top_point += origin_point;
        if(ego_box.HasOverlap(right_bound)){
          ADEBUG << std::setprecision(9) << "ego collision: right top { " << right_top.x() << "," << right_top.y() << " }";
          ADEBUG << std::setprecision(9) << "ego collision: right down { " << right_down.x() << "," << right_down.y() << " }";
          ADEBUG << "ego collision: right side corner collides with the parking line. >>>>>";
          double off_set = std::min(std::fabs(right_top.x() - ego_box.max_x()), std::fabs(right_top.x() - ego_box.min_x()));
          right_top.set_x(right_top.x() + off_set + 0.5);
          right_down.set_x(right_down.x() + off_set + 0.5);
        }
      }
    }
    //left_down.set_y(left_down.y() - 1);
    //right_down.set_y(right_down.y() - 1);
   // ADEBUG << "COLLISION:" << left_down.x() << " " << left_down.y();
  }

//NEW ADD  HORIZONTAL PARKING
  double buff = 1;
  if(parking_type_ == HORIZONTAL){
    left_down.set_x(left_down.x() + buff);
    right_down.set_x(right_down.x() + buff);
    left_top.set_x(left_top.x() + buff);
    right_top.set_x(right_top.x() + buff);
  }
...
////    left_down.set_x(left_down.x() - 0.3);
  //  double buff = 1.5;
  //  double offset = vehicle_state_.x() - origin_point.x();
  //  if(offset <= 1){
  //    left_top.set_x(left_top.x() + offset - buff);
  //    left_down.set_x(left_down.x() + offset - buff);
  //  }else if(offset >= 1.6){
  ...
  GetRoadBoundary(frame, nearby_path, center_line_s, origin_point, origin_heading,
  ...
  if (average_l < 0) {
    ...
    if(parking_type_ == HORIZONTAL){
      ...
      for (size_t i = 0; i < point_size; i++) {
        right_lane_boundary[i].set_x(right_lane_boundary[i].x() + buff);
      }
    }
...
//    }
    ...
    for (size_t i = 0; i < point_left_to_left_top_connor_index; i++) {
      ...
      ADEBUG << "right_segment x:" << right_lane_boundary[i].x() << " y:" << right_lane_boundary[i].y();
    }
    ...
    ADEBUG<< "left_top x:" << left_top.x() << " y:" << left_top.y();
    ADEBUG<< "left_down x:" << left_down.x() << " y:" << left_down.y();
    ADEBUG<< "right_down x:" << right_down.x() << " y:" << right_down.y();
    ADEBUG<< "right_top x:" << right_top.x() << " y:" << right_top.y();
    ...
    //NEW ADD 
    std::vector<Vec2d> boundart_segment{right_lane_boundary[right_lane_boundary_last_index],
                                 left_lane_boundary[left_lane_boundary_last_index]};
    roi_parking_boundary->push_back(boundart_segment);

    for (size_t i = left_lane_boundary_last_index; i > 0; i--) {
      std::vector<Vec2d> segment{left_lane_boundary[i],
                                 left_lane_boundary[i - 1]};
     // left_lane_boundary[i].set_x(left_lane_boundary[i].x() + 4);
      roi_parking_boundary->push_back(segment);
    }
 ...
 //            << ", " << left_lane_boundary[i].y();
  ...
  ADEBUG << "ROI: " << ROI_xy_boundary[0] << " " << ROI_xy_boundary[1] << " " << ROI_xy_boundary[2] << " " << ROI_xy_boundary[3];
...
//        point, 10.0, vehicle_state_.heading(), M_PI / 1.5, &nearest_lane,
...
//    }
      if(frame->local_view().pad_msg->appmode() == 4){
        common::PointENU summon_point;
        summon_point.set_x((frame->local_view().pad_msg->parking_space_info().point_left_up().x()
                            + frame->local_view().pad_msg->parking_space_info().point_right_up().x())/2);
        summon_point.set_y((frame->local_view().pad_msg->parking_space_info().point_left_up().y()
                            + frame->local_view().pad_msg->parking_space_info().point_right_up().y())/2);
        double summon_s = 0.0;
        double summon_l = 0.0;
        int statu = HDMapUtil::BaseMap().GetNearestLane(summon_point, &nearest_lane, &summon_s, &summon_l);
        if(statu != 0){
          ADEBUG << "Summon Find Nearest Lane Failed";
        }
      }else{
        for (auto& ref_line_info : *frame->mutable_reference_line_info()){
          Vec2d vehicle_point(vehicle_state_.x(), vehicle_state_.y());
          for (const auto& segment : ref_line_info.Lanes()) {
            if(segment.lane->IsOnLane(vehicle_point)){
              nearest_lane = hdmap_->GetLaneById(segment.lane->id());
              break;
            }
          }
        }
  ...
  //NEW ADD
  // if(nearest_lane->junctions().size() > 0){
  //   auto junction_id = HDMapUtil::BaseMap().GetJunctionById(nearest_lane->junctions()[0]->id());

  // }
  frame->mutable_open_space_info()->set_target_parking_lane(nearest_lane);
//  ADEBUG << "START LANE ID:" << nearest_lane->lane().id().id();
...
//  while(distance < 10){
    int pre_lanes_num = nearest_lane->lane().predecessor_id_size();
    if(pre_lanes_num != 0){
      for(int i = 0; i < pre_lanes_num; ++i){
        auto pre_lane_id = nearest_lane->lane().predecessor_id(i);
        auto pre_lane = hdmap_->GetLaneById(pre_lane_id);
        ...
        if(pre_lane->lane().turn() == 1){
          ...
          if(pre_lane->lane().left_neighbor_reverse_lane_id().empty()){
            auto pre_pre_lane_id = pre_lane->lane().predecessor_id(0);
            auto pre_pre_lane = hdmap_->GetLaneById(pre_pre_lane_id);
            LaneSegment pre_pre_lanesegment =
                LaneSegment(pre_pre_lane, pre_pre_lane->accumulate_s().front(),
                            pre_pre_lane->accumulate_s().back());
            segments_vector.push_back(pre_pre_lanesegment);
            //break;
          }
          segments_vector.push_back(pre_lanesegment);
...
//  }
  ...
  if (next_lanes_num != 0) {
    for (int i = 0; i < next_lanes_num; ++i) {
      ...
      if(next_lane->total_length() < 18){
        auto next_next_lane_id = next_lane->lane().successor_id(0);
        auto next_next_lane = hdmap_->GetLaneById(next_next_lane_id);
        LaneSegment next_next_lanesegment =
            LaneSegment(next_next_lane, next_next_lane->accumulate_s().front(),
                      next_next_lane->accumulate_s().back());
        segments_vector.push_back(next_next_lanesegment);
      }
...
// 晶众厂区地图停车位
   ...
   Vec2d right_top;
//  if(frame->local_view().pad_msg->appmode() != 2){
    left_top.set_x(frame->local_view().pad_msg->parking_space_info().point_left_up().x());
    left_top.set_y(frame->local_view().pad_msg->parking_space_info().point_left_up().y());
    left_down.set_x(frame->local_view().pad_msg->parking_space_info().point_left_down().x());
    left_down.set_y(frame->local_view().pad_msg->parking_space_info().point_left_down().y());
    right_down.set_x(frame->local_view().pad_msg->parking_space_info().point_right_down().x());
    right_down.set_y(frame->local_view().pad_msg->parking_space_info().point_right_down().y());
    right_top.set_x(frame->local_view().pad_msg->parking_space_info().point_right_up().x());
    right_top.set_y(frame->local_view().pad_msg->parking_space_info().point_right_up().y());
    ADEBUG << "Slot_det_parking";
    ADEBUG << "target_parking_spot_id_:" << target_parking_spot_id_;
    ADEBUG << std::setprecision(10)<<"LEFT top X:" << left_top.x();
    ADEBUG << std::setprecision(10)<<"LEFT top Y:" << left_top.y();
    ADEBUG << std::setprecision(10)<<"LEFT down X:" << left_down.x();
    ADEBUG << std::setprecision(10)<<"LEFT down Y:" << left_down.y();
    ADEBUG << std::setprecision(10)<<"RIGHT down X:" << right_down.x();
    ADEBUG << std::setprecision(10)<<"RIGHT down Y:" << right_down.y();
    ADEBUG << std::setprecision(10)<<"RIGHT top X:" << right_top.x();
    ADEBUG << std::setprecision(10)<<"RIGHT top Y:" << right_top.y();
    //  }else{
//    for(auto slot : frame->local_view().valid_slots->slotsum()){
//      if(target_parking_spot_id_ == slot.id_v().id()){
//        left_top.set_x(slot.polygon_v().point(0).x());
//        left_top.set_y(slot.polygon_v().point(0).y());
//        left_down.set_x(slot.polygon_v().point(1).x());
//        left_down.set_y(slot.polygon_v().point(1).y());
//        right_down.set_x(slot.polygon_v().point(2).x());
//        right_down.set_y(slot.polygon_v().point(2).y());
//        right_top.set_x(slot.polygon_v().point(3).x());
//        right_top.set_y(slot.polygon_v().point(3).y());
        //left_top(slot.polygon_v().point(0).x(), slot.polygon_v().point(0).y());
        //left_down(slot.polygon_v().point(1).x(), slot.polygon_v().point(1).y());
        //right_down(slot.polygon_v().point(2).x(), slot.polygon_v().point(2).y());
        //right_top(slot.polygon_v().point(3).x(), slot.polygon_v().point(3).y());
//        break;
//      }
//    }
//  }
...
//  if(frame->local_view().pad_msg->appmode() == 4){
//    common::PointENU summon_point;
//    summon_point.set_x(left_top.x());
//    summon_point.set_y(left_top.y());
...
//NEW ADD 检测点是否在路口区域
bool OpenSpaceRoiDecider::CheckIsinJunctionByS(
  const hdmap::Path &nearby_path, const double check_point_s){
//  for(const auto & junction : nearby_path.junction_overlaps()){
//    if(check_point_s <= junction.end_s && check_point_s >= junction.start_s){
//      return true;
//    }
//  }
  if(nearby_path.GetRoadLeftWidth(check_point_s) < 1.6){
    return true;
  }
  return false;
}
