...
class OpenSpaceRoiDecider : public Decider {
 ...
 private:
  jmc_auto::common::Status Process(Frame *frame) override;
 ...
 private:
  ...
  void SummonDirection(Frame *const frame, const std::array<common::math::Vec2d, 4> &vertices);
//NEW ADD  HORIZONTAL PARKING
  void CheckParkingType(Frame *const frame, const std::array<common::math::Vec2d, 4> &vertices);
  //NEW ADD CHECK NEED TRUNCATE
  void CheckNeedTruncate(Frame *const frame);
  ...
  //NEW ADD  if not inn junction, return false
  bool CheckIsinJunctionByS(
 ...
 private:
  ...
  jmc_auto::common::VehicleParam vehicle_params_;

  ThreadSafeIndexedObstacles *obstacles_by_frame_;

  common::VehicleState vehicle_state_;
  SummonType summon_direction_ = UNKNOW;
  Parking_Type parking_type_;
