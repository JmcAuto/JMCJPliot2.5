...
bool PathDecider::MakeStaticObstacleDecision(
  ...
  for (const auto *obstacle : path_decision->obstacles().Items()) {
    ...
    if (!obstacle->IsStatic()) {
      //NEW ADD
      if(obstacle->speed() >= 0.11){
      const auto& adc_sl = reference_line_info_->AdcSlBoundary();
      double dis = obstacle->PerceptionSLBoundary().start_s() - adc_sl.end_s();
      ADEBUG << "dis:" << dis << " obs_sl:" << obstacle->PerceptionSLBoundary().start_l() << " adc_sl:" << adc_sl.start_l(); 
      if(dis <= 6 && dis >= -0.3){
        if(!(obstacle->PerceptionSLBoundary().start_l() > adc_sl.end_l() || obstacle->PerceptionSLBoundary().end_l() < adc_sl.start_l())){
        ObjectDecisionType object_decision;
        ObjectStop object_stop;
        object_stop.set_reason_code(StopReasonCode::STOP_REASON_OBSTACLE);
        object_stop.set_distance_s(-dis);

        const double stop_ref_s =
        obstacle->PerceptionSLBoundary().start_s() - dis + 0.1;
        ...
        *object_decision.mutable_stop() = object_stop;
        path_decision->AddLongitudinalDecision("PathDecider/dynamic_obstacle too close",
...
//      }
      ...
      const auto& adc_sl = reference_line_info_->AdcSlBoundary();
      double dis = obstacle->PerceptionSLBoundary().start_s() - adc_sl.end_s();
      //double overlap_distance = obstacle->PerceptionSLBoundary().end_s() - obstacle->PerceptionSLBoundary().start_s();
      ADEBUG << "dis:" << dis << " obs_sl:" << obstacle->PerceptionSLBoundary().start_l() << " adc_sl:" << adc_sl.start_l(); 
     if(dis <= 9 && dis >= -0.5){
        if(obstacle_type_name == "PEDESTRIAN" && dis > 6){
          ADEBUG << "PEDESTRIAN too far";
          continue;
        }
        if(!(obstacle->PerceptionSLBoundary().start_l() > adc_sl.end_l() || 
            obstacle->PerceptionSLBoundary().end_l() < adc_sl.start_l())){
        ObjectDecisionType object_decision;
        ObjectStop object_stop;
        object_stop.set_reason_code(StopReasonCode::STOP_REASON_OBSTACLE);
        object_stop.set_distance_s(-dis);

        const double stop_ref_s = adc_sl.end_s() + 0.01;
//        obstacle->PerceptionSLBoundary().start_s() - 4;
        const auto stop_ref_point =
        reference_line_info_->reference_line().GetReferencePoint(stop_ref_s);
        object_stop.mutable_stop_point()->set_x(stop_ref_point.x());
        object_stop.mutable_stop_point()->set_y(stop_ref_point.y());
        object_stop.set_stop_heading(stop_ref_point.heading());
        *object_decision.mutable_stop() = object_stop;
        path_decision->AddLongitudinalDecision("PathDecider/obstacle too close",
                                             obstacle->Id(), object_decision);
        }
      }
      // else if(dis >= -overlap_distance && dis < 0){
      //   if(!(obstacle->PerceptionSLBoundary().start_l()-0.2 > adc_sl.end_l() ||
      //        obstacle->PerceptionSLBoundary().end_l()+0.2 < adc_sl.start_l())){
      //     ObjectDecisionType object_decision;
      //   ObjectStop object_stop;
      //   object_stop.set_reason_code(StopReasonCode::STOP_REASON_OBSTACLE);
      //   object_stop.set_distance_s(-dis);
      ...
      //   const double stop_ref_s =
      //   obstacle->PerceptionSLBoundary().start_s() - dis + 0.5;
      //   const auto stop_ref_point =
      //   reference_line_info_->reference_line().GetReferencePoint(stop_ref_s);
      //   object_stop.mutable_stop_point()->set_x(stop_ref_point.x());
      //   object_stop.mutable_stop_point()->set_y(stop_ref_point.y());
      //   object_stop.set_stop_heading(stop_ref_point.heading());
      //   *object_decision.mutable_stop() = object_stop;
      //   path_decision->AddLongitudinalDecision("PathDecider/obstacle lateral too close",
      //                                        obstacle->Id(), object_decision);
      //   }
      // }
  }
  ADEBUG << "Obstacles nums:" << count;
