...
#include "modules/planning/proto/planning_cmd.pb.h"
...
class OpenSpaceTrajectoryOptimizer {
 public:
  ...
  common::Status Plan(
      ...
      const std::vector<std::vector<common::math::Vec2d>>&
          obstacles_vertices_vec, const bool mode,
      double* time_latency);
 ...
 //NEW ADD 
  ...
  const std::pair<int, bool> current_partition();
  bool destination_replan() {return is_destination_replan_;}
  bool need_change_end_node() {return need_change_end_node_;}
  void set_need_change_end_node(const bool flag) {need_change_end_node_ = flag;}
  ...
  void UpdateDebugInfo(
      ::jmc_auto::planning_internal::OpenSpaceDebug* open_space_debug);

//  bool IsStartNodeCollison(){return start_node_collision_;}

  jmc_auto::planning_internal::OpenSpaceDebug* mutable_open_space_debug() {
 ...
 private:
  ...
  FILE* PrintCsvLogs(FILE* file, std::string name, const SimpleTrajectoryDebug *debug);
  FILE* CloseCsvLogs(FILE* file);
 private:
  ...
  bool is_first_ = true;
  bool is_last_ = false;
  std::pair<int, bool> current_partition_;
  jmc_auto::planning_internal::OpenSpaceDebug open_space_debug_;
  std::vector<HybridAStartResult> partition_trajectories_;
//  bool start_node_collision_ = false;
  bool is_destination_replan_ = false;

  FILE *raw_trajectory_log_file_ = nullptr;
  FILE *smoothed_trajectory_log_file_ = nullptr;

  bool need_change_end_node_ = false;
};
}  // namespace planning
}  // namespace jmc_auto
