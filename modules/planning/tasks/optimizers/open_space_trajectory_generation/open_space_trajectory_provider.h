...
struct OpenSpaceTrajectoryThreadData {
  ...
  double rotate_angle;
  jmc_auto::common::math::Vec2d translate_origin;
...
class OpenSpaceTrajectoryProvider : public TrajectoryOptimizer {
 ...
 private:
  jmc_auto::common::Status Process() override;
...
//NEW ADD
  void GenerateStraightLineTrajectory(DiscretizedTrajectory* const trajectory_data, 
  ...
  void DealWithShortSummon(DiscretizedTrajectory* const trajectory_data);
 
private:
  bool thread_init_flag_ = false;
  bool is_trajectory_truncated_ = false;
  ...
  //NEW ADD
  std::atomic<bool> is_straightline_trajectory_{false};
  std::atomic<bool> is_secondend_trajectory_{false};
