...
//  current_partitioned_ = previous_frame->current_partitioned();
//  if(frame_->current_partitioned().second){
//    current_partitioned_ = frame_->current_partitioned().first;
//  }else if(frame_->current_partitioned().first > 0){
//    current_partitioned_ = frame_->current_partitioned().first - 1;
//  }else{
    current_partitioned_ = partitioned_trajectories->size() - 1;
//  }
 // for (size_t i = 0; i < trajectories_size; ++i) {
{
    const auto& gear = partitioned_trajectories->at(current_partitioned_).second;
    const auto& trajectory = partitioned_trajectories->at(current_partitioned_).first;
    size_t trajectory_size = trajectory.size();
    CHECK_GT(trajectory_size, 0);

    flag_change_to_next = CheckReachTrajectoryEnd(
        trajectory, gear, trajectories_size, current_partitioned_, &current_trajectory_index,
...
//    ADEBUG << "current_partitioned_:" << current_partitioned_;
  if (!flag_change_to_next){
...
//        }
    ...
    if (!closest_point.empty()) {
      ...
      closest_point_on_trajs.emplace(std::make_pair(current_partitioned_, closest_point_index),
                                     max_iou_ratio);
    }
  }
}
  if (!flag_change_to_next) {
    ...
    if (use_fail_safe_search) {
     ...
     //   return Status(ErrorCode::PLANNING_ERROR, msg);
       current_trajectory_index = current_partitioned_;
       const auto& trajectory = partitioned_trajectories->at(current_partitioned_).first;
       current_trajectory_point_index = trajectory.size() - 1;
     //}
    }
  }
...
//NEW ADD
 if(frame_->check_end() && previous_frame->current_partitioned().second){
// if(frame_->second_parking() && previous_frame->current_partitioned().second){
    frame_->set_current_partitioned(previous_frame->current_partitioned().first + 1, false);
 }
 ADEBUG << "dest replan: current_part_num: " << frame_->current_partitioned().first << " traj bool: " << frame_->current_partitioned().second;
  ...
  AdjustRelativeTimeAndS(open_space_info.partitioned_trajectories(),
                         ...
                         chosen_partitioned_trajectory);
//NEW ADD
 // frame_->set_current_partitioned(current_trajectory_index);
 // if(frame_->second_parking()){
 //   frame_->set_current_partitioned(0, false);
 // }
  ADEBUG << "current_trajectory_index:" << current_trajectory_index;
  return Status::OK();
}
...
//NEW ADD
//    ADEBUG << "POINT: X:" << stitched_trajectory_result.at(i).path_point().x() << " y:" << stitched_trajectory_result.at(i).path_point().y() << " theta:" << stitched_trajectory_result.at(i).path_point().theta() << " kappa:" << stitched_trajectory_result.at(i).path_point().kappa();
...
//                                         wheel_base_)
//  }
  //ADEBUG << "kappa:" << point->path_point().kappa();
...
bool OpenSpaceTrajectoryPartition::CheckReachTrajectoryEnd(
  ...
  const double traj_end_point_moving_direction =
   ...
   double heading_search_to_trajs_end = std::abs(NormalizeAngle(
      traj_end_point_moving_direction - vehicle_moving_direction_));
  //NEW ADD
  //if(heading_search_to_trajs_end > 0.3) heading_search_to_trajs_end = 3.14 - heading_search_to_trajs_end;
ADEBUG << "end_theta:" << path_end_point_theta << " traj_direction" << traj_end_point_moving_direction;
  ...
  if (lateral_offset < lateral_offset_to_midpoint_ &&
   ...
   //    Polygon2d(ego_box_).ComputeIoU(Polygon2d(path_end_point_box));
    ...
    //if (end_point_iou_ratio > vehicle_box_iou_threshold_to_midpoint_) {
      ...
      ADEBUG << "dest replan: Reach the end of a trajectory, switching to next one";
      frame_->set_check_end(true);
      //frame_->set_current_partitioned(1);
      return true;
   // }
  }
  //frame_->set_check_end(false);
//  if(ego_v_ < 0.01 && frame_->current_partitioned().second){
//    bool is_short_path = false;
//    if(trajectory_size < 80){
//      is_short_path = true;
//    }
//    if(!is_short_path){
//      if(distance_to_trajs_end < 0.5){
//        frame_->set_second_parking(true);
//        ADEBUG << "replanning_parking";
//      }
//    }else{
//      const TrajectoryPoint& trajectory_start_point = trajectory.front();
//      const PathPoint& path_start_point = trajectory_start_point.path_point();
//      const double path_start_point_x = path_start_point.x();
//      const double path_start_point_y = path_start_point.y();
//      const double distance_to_trajs_start_end =
//      std::sqrt((path_end_point_x - path_start_point_x) * (path_end_point_x - path_start_point_x) +
//                (path_end_point_y - path_start_point_y) * (path_end_point_y - path_start_point_y));
//      if(distance_to_trajs_end < distance_to_trajs_start_end / 3){
//        frame_->set_second_parking(true);
//        ADEBUG << "replanning_parking";
//      }
//    }
//  }
  ADEBUG << "Vehicle did not reach end of a trajectory with conditions for "
         ...
         << (longitudinal_offset < longitudinal_offset_to_midpoint_)
         << " and actual longitudinal distance: " << longitudinal_offset << longitudinal_offset_to_midpoint_
         << "; heading_check: "
         << (heading_search_to_trajs_end < heading_offset_to_midpoint_)
