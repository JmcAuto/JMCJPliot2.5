...
using jmc_auto::common::color::ANSI_GREEN;
using jmc_auto::common::color::ANSI_RED;
using jmc_auto::common::color::ANSI_RESET;
using jmc_auto::common::util::PrintIter;
using jmc_auto::hdmap::HDMap;
using jmc_auto::hdmap::HDMapUtil;
using jmc_auto::hdmap::SignalInfoConstPtr;
using jmc_auto::localization::LocalizationEstimate;
using jmc_auto::perception::TrafficLight;
...
bool GetAllTrafficLights(std::vector<SignalInfoConstPtr> *traffic_lights) {
  static auto map_filename = jmc_auto::hdmap::BaseMapFile();
  static jmc_auto::hdmap::Map map_proto;
  ...
  if (map_proto.lane_size() == 0 && map_traffic_lights.empty()) {
    ...
    if (jmc_auto::common::util::EndWith(map_filename, ".xml")) {
      if (!jmc_auto::hdmap::adapter::OpendriveAdapter::LoadData(map_filename,
                                                              &map_proto)) {
        return false;
      }
    } else if (!jmc_auto::common::util::GetProtoFromFile(map_filename,
                                                       &map_proto)) {
      return false;
    }
