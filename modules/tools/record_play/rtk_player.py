...
JMC_AUTO_ROOT = os.path.join(os.path.dirname(__file__), '../../../')
...
class RtkPlayer(object):
    ...
    def __init__(self, record_file, speedmultiplier, completepath, replan):
        ...
        self.planning_pub = rospy.Publisher(
            '/jmc_auto/planning', planning_pb2.ADCTrajectory, queue_size=1)
...
def main():
    ...
    Logger.config(
        log_file=os.path.join(JMC_AUTO_ROOT, 'data/log/rtk_player.log'),
        use_stdout=True,
        log_level=logging.DEBUG)

    record_file = os.path.join(JMC_AUTO_ROOT, 'data/log/garage.csv')
    player = RtkPlayer(record_file, args['speedmulti'],
                       args['complete'].lower(), args['replan'].lower())
    atexit.register(player.shutdown)

    rospy.Subscriber('/jmc_auto/canbus/chassis', chassis_pb2.Chassis,
    ...
    rospy.Subscriber('/jmc_auto/control/pad', pad_msg_pb2.PadMessage,
