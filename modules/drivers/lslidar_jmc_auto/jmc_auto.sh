  ...
  case $OP_SYSTEM in
    "Linux")
      ...
      if (( $(echo "$MEM_SIZE < $DEFAULT_MEM_SIZE" | bc -l) )); then
         warning "System memory [${MEM_SIZE}G] is lower than minimum required memory size [2.0G]. JmcAuto build could fail."
      fi
      ;;
    "Darwin")
...
function build() {
  ...
  if [ -d /jmc_auto-simulator ] && [ -e /jmc_auto-simulator/build.sh ]; then
    cd /jmc_auto-simulator && bash build.sh build
    if [ $? -ne 0 ]; then
      fail 'Build failed!'
    fi
  fi
...
function warn_proprietary_sw() {
  ...
  echo -e "${RED}and is not covered by the license terms of the jmc_auto project${NO_COLOR}"
  echo -e "${RED}(see file license).${NO_COLOR}"
}

function release() {
  RELEASE_DIR="${HOME}/.cache/jmc_auto_release"
  if [ -d "${RELEASE_DIR}" ]; then
    rm -rf "${RELEASE_DIR}"
  fi
  JMC_AUTO_RELEASE_DIR="${RELEASE_DIR}/jmcauto"
  mkdir -p "${JMC_AUTO_RELEASE_DIR}"
  ...
  for BIN in ${BINARIES}; do
    ...
    DST_PATH="${JMC_AUTO_RELEASE_DIR}/${BIN}"
    if [ -e "${SRC_PATH}" ]; then
  ...
  for conf in $CONFS; do
    mkdir -p $JMC_AUTO_RELEASE_DIR/$conf
    rsync -a $conf/* $JMC_AUTO_RELEASE_DIR/$conf
  done
  for data in $DATAS; do
    mkdir -p $JMC_AUTO_RELEASE_DIR/$data
    if [ $data != "modules/map/data" ]; then
      rsync -a $data/* $JMC_AUTO_RELEASE_DIR/$data
    fi
  done
  # Other
  for path in "${OTHER[@]}"; do
    mkdir -p $JMC_AUTO_RELEASE_DIR/$path
    rsync -a $path/* $JMC_AUTO_RELEASE_DIR/$path
  done

  # dreamview frontend
  cp -a modules/dreamview/frontend $JMC_AUTO_RELEASE_DIR/modules/dreamview

  # remove all pyc file in modules/
  find modules/ -name "*.pyc" | xargs -I {} rm {}

  # scripts
  cp -r scripts ${JMC_AUTO_RELEASE_DIR}

  # lib
  LIB_DIR="${JMC_AUTO_RELEASE_DIR}/lib"
  mkdir "${LIB_DIR}"
  if $USE_ESD_CAN; then
    warn_proprietary_sw
    for m in esd_can
  ...
  cp /jmcauto/modules/perception/cuda_util/cmake_build/libcuda_util.so $LIB_DIR

  # doc
  cp -r docs "${JMC_AUTO_RELEASE_DIR}"
  cp LICENSE "${JMC_AUTO_RELEASE_DIR}"
  cp third_party/ACKNOWLEDGEMENT.txt "${JMC_AUTO_RELEASE_DIR}"

  # release info
  META="${JMC_AUTO_RELEASE_DIR}/meta.ini"
...
function run_test() {
  ...
  if [ -d /jmc_auto-simulator ] && [ -e /jmc_auto-simulator/build.sh ]; then
      cd /jmc_auto-simulator && bash build.sh test
      if [ $? -ne 0 ]; then
        fail 'Test failed!'
        return 1
      fi
...
function run_bash_lint() {
  FILES=$(find "${JMC_AUTO_ROOT_DIR}" -type f -name "*.sh" | grep -v ros)
...
function build_gnss() {
  ...
  if [ -d "${ROS_ROOT}" ]; then
    ...
    warning "ROS not found. Run jmc_auto.sh build first."
    exit 1
  fi
...
function config() {
  ${JMC_AUTO_ROOT_DIR}/scripts/configurator.sh
}
