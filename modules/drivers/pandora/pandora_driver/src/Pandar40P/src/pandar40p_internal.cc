...
void Pandar40P_Internal::RecvTask() {
  ...
  while (enable_lidar_recv_thr_) {
    ...
    if (rc == -1) {
      std::cout << "Get PandarPacket error" <<std::endl;
      continue;
    }

    //std::cout << "Get PandarPacket successed,size of packet is :"<< pkt.size << std::endl;
    ...
    if (pkt.size == GPS_PACKET_SIZE) {
      ...
      if (ret == 0) {
        ProcessGps(gpsMsg);
        std::cout<<"Get GPS msg successed,gpsMsg is"<<gpsMsg.year<<"-"<<gpsMsg.month<<"-"
                  <<gpsMsg.day<<"-"<<gpsMsg.hour<<"-"<<gpsMsg.minute<<"-"<<gpsMsg.second<<std::endl;
      }
      else{std::cout<<"Get GPS_MSG error"<<std::endl;}
      continue;
    }
