...
bool PncMap::UpdateVehicleState(const VehicleState &vehicle_state) {
  ...
  if (route_index < 0 ||
    ...
    AERROR<<"route_index "<<route_index;
    AERROR<<"static_cast<int>(route_indices_.size()) "<<static_cast<int>(route_indices_.size());
...
bool PncMap::GetNearestPointFromRouting(const VehicleState &state,
  ...
  const double kHeadingBuffer = M_PI / 10;
  ...
  const int status = hdmap_->GetLanesWithHeading(
      point, kMaxDistance, state.heading(), M_PI / 2.0 + kHeadingBuffer, &lanes);
  if (status < 0) {
