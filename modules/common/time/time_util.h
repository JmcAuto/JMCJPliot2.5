...
class TimeUtil {
 public:
  ...
  // @brief: GPS 周内�?timestamp to UNIX timestamp, in seconds.
  template <typename T>
  static T Gpsweek2unix(T gps_time,T unix_time) {
   T gps=Unix2gps(unix_time);
   gps_time=int(gps/WEEK_SECOND)*WEEK_SECOND+gps_time;
 ...
 private:
  ...
  static const int WEEK_SECOND = 604800;
