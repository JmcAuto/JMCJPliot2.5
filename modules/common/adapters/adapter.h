 ...
 * abstraction for JmcAuto modules to interact with various I/O (e.g.
 * ROS). The adapter will also store history data, so that other
 * JmcAuto modules can get access to both the current and the past data
...
class Adapter : public AdapterBase {
 public:
  ...
  typedef std::shared_ptr<D> DataStdPtr;
...
template<typename T>
typename std::shared_ptr<T> to_std(typename boost::shared_ptr<T const> const& p)
{
    return
        std::shared_ptr<T>(
                const_cast<T*>(&(*p)));
//                boost::bind(&do_release<T>, p, _1));
  ...
  DataStdPtr GetLatestObservedStdPtr() {
    std::lock_guard<std::mutex> lock(mutex_);
    DCHECK(!observed_queue_.empty())
        << "The view of data queue is empty. No data is received yet or you "
           "forgot to call Observe()"
  ...
  double GetDelaySec() const override {
    if (last_receive_time_ == 0) {
      ...
      return jmc_auto::common::time::Clock::NowInSeconds() - last_receive_time_;
    }
  }
 ...
 private:
  ...
  bool FeedFile(const std::string& message_file, IdentifierType<T>) {
    D data;
    if (!jmc_auto::common::util::GetProtoFromFile(message_file, &data)) {
  ...
  void RosCallback(DataPtr message) {
    last_receive_time_ = jmc_auto::common::time::Clock::NowInSeconds();
    EnqueueData(message);
