...
void ProcessMonitor::RunOnce(const double current_time) {
  ...
  for (const auto &proc : procs) {
    // Judge if user's process
    const auto gid_file = common::util::StrCat("/proc/", proc, "/gid_map");
    if (0 == access(gid_file.data(), W_OK)) {
...
void ProcessMonitor::UpdateModule(
  ...
  if (status->process_status().running()) {
    ...
    const std::string msg = jmc_auto::common::util::StrCat(
