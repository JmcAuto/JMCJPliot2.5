...
PathPoint TrajectoryAnalyzer::QueryPurePathPoint(const double x, 
                                                const double y,
                                                const double v_ ,
                                                const double kv_ ,
                                                const double Ld0_) const {
  double d_min = PointDistanceSquare(trajectory_points_.front(), x, y);
  size_t index_min = 0;
  ...
  for (size_t i = 1; i < trajectory_points_.size(); ++i) {
   ...
   double start_index = index_min ;
   double Lf = kv_ * v_ + Ld0_ ;//预瞄距离
   double L = 0.0 ;
   while ((Lf > L) && (start_index +1) < trajectory_points_.size()){
     L = std::sqrt(PointDistanceSquare(trajectory_points_[start_index+1],
                                        trajectory_points_[index_min].path_point().x(),
                                        trajectory_points_[index_min].path_point().y())) ;
     ++start_index ;
   }
   return TrajectoryPointToPathPoint(trajectory_points_[start_index-1]);
...
TrajectoryPoint TrajectoryAnalyzer::QueryNearestPointAndIndexByPosition(
    const double x, const double y ,int & index) const {
  double d_min = PointDistanceSquare(trajectory_points_.front(), x, y);
  size_t index_min = 0;
...
TrajectoryPoint TrajectoryAnalyzer::QueryPreviewPointByPosition(
    const double x , const double y,const int preview_window) const{
    int point_index = 0 ;
    TrajectoryPoint target_point = QueryNearestPointAndIndexByPosition(x,y,point_index);
    ADEBUG << "point_index " << point_index ;
    point_index += preview_window ;
    ADEBUG << "after preview point_index " << point_index ;
    if (point_index + 2 < trajectory_points_.size())
    {
      return trajectory_points_[point_index] ;
    }else{
      return trajectory_points_[trajectory_points_.size()-2];
    }
}
