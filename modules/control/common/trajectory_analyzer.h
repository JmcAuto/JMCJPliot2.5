...
class TrajectoryAnalyzer {
 public:
  ...
  common::TrajectoryPoint QueryNearestPointAndIndexByPosition(const double x,
                                           const double y,int & index) const ;
  /*根据当前位置匹配最近点，再在该点的基础上往前10个点*/                                  
  common::TrajectoryPoint QueryPreviewPointByPosition(const double x, const double y,const int preview_window) const;
  /**
  ...
  /**
   * @brief find pure pursuit point
   * @param x value of x-coordination in the given position
   * @param y value of y-coordination in the given position
   * @return a point on trajectory, the point may be a point of trajectory
   * or interpolated by two adjacent points of trajectory
   */
  common::PathPoint QueryPurePathPoint(const double x, 
                                        const double y,
                                        const double v_,
                                        const double kv_,
