...
*/
  ...
  && std::fabs(debug -> path_remain()) < 0.15)
  {
...
//    ADEBUG << "enter the driving mode";
    ...
    ADEBUG << "两帧距离差值：" << previous_stop_distance - std::fabs(debug -> path_remain());
/*
    if ( previous_mode && std::abs(previous_stop_distance - debug -> path_remain()) > 0.3
    && debug -> path_remain() < FLAGS_stop_path_remain)
    {
      ADEBUG << "召出急刹优化模式";
      debug -> set_path_remain(0.69);
      previous_stop_distance = 0.691;
    }
*/
    if(std::fabs(debug -> path_remain()) > 10)
    {
    ...
    if( !trajectory_message_ -> decision().main_decision().has_parking()
  && !trajectory_message_ -> decision().main_decision().has_summon() && std::fabs(debug -> path_remain()) < 8)
    {
      speed_cmd = 0.8;
    }

    if(std::fabs(debug -> path_remain()) < 3)
    {
      speed_cmd = 0.5;
    }
...
//轮速更新距离在bool值为true时才可用
//同一段轨迹中，当前后两帧的距离差超过30cm时，不再用轮速传感器更新行车距离，拿最新的规划距离
// if (!(std::fabs(debug -> path_remain() - previous_stop_distance) > 0.3)){
      if ( FLAGS_enable_last_distance_by_wheelpulse  && chassis->gear_location() == previous_gear
      && (std::fabs(debug -> path_remain()) - previous_stop_distance) < 0.3)
      {
        //直行模式或者非水平车位都是60cm内使用轮速脉冲计算行车距离
        //水平车位在20cm内使用轮速脉冲计算行车距离
        if((!trajectory_message_ -> decision().main_decision().has_parking()
        && !trajectory_message_ -> decision().main_decision().has_summon())
        || (trajectory_message_ -> decision().main_decision().has_parking()
        && !(trajectory_message_ -> decision().main_decision().parking().parking_type() == planning::Parking_Type::HORIZONTAL))
        || (trajectory_message_ -> decision().main_decision().has_parking()
        && trajectory_message_ -> decision().main_decision().parking().parking_type() == planning::Parking_Type::HORIZONTAL
        && std::fabs(debug -> path_remain()) < 0.2))
        {
...
//      }
  ...
  */
  ...
  if( std::fabs(debug -> path_remain()) < FLAGS_stop_path_remain
...
//档位控制，只有停车状态或者空档状态下才能换档
  ...
  && std::fabs(debug -> path_remain()) > FLAGS_stop_path_remain))
  {
    ...
    previous_stop_distance = std::fabs(debug->path_remain());
...
/*  if ( trajectory_message_ -> decision().main_decision().has_summon() )
  {
    previous_mode = true;
  }
  else
