...
void ControllerAgent::RegisterControllers(const ControlConf *control_conf) {
  AINFO << "Only support Lat + Lon + PidStantly controllers as of now";
  for (auto active_controller : control_conf->active_controllers()) {
    switch (active_controller) {
      ...
      case ControlConf::LON_CONTROLLER:
        ...
        break;
      case ControlConf::PID_STANTLY_CONTROLLER:
        controller_factory_.Register(
            ControlConf::PID_STANTLY_CONTROLLER,
            []() -> Controller * { return new PidStantlyController(); });
        break;
      case ControlConf::PURE_CONTROLLER:
          controller_factory_.Register(
              ControlConf::PURE_CONTROLLER,
              []() -> Controller* { return new PureController(); });
          break;
