...
generate_map() {
    xy_file=$1
    dir_name="${JMC_AUTO_ROOT_DIR}/modules/map/data/$2"
    #left_num=$3
    #right_num=$4
    if [ -d ${dir_name} ]; then
        rm -rf ${dir_name}.last
    ...
    mkdir -p ${dir_name}
    python ${JMC_AUTO_ROOT_DIR}/modules/tools/map_gen/map_gen_single_lane.py $xy_file $dir_name/base_map.txt 1.0
    echo "--map_dir=${dir_name}" >> modules/common/data/global_flagfile.txt
    bash ${JMC_AUTO_ROOT_DIR}/scripts/generate_routing_topo_graph.sh 
    ${JMC_AUTO_ROOT_DIR}/bazel-bin/modules/map/tools/sim_map_generator --map_dir=${dir_name} --output_dir=${dir_name}
}
