...
GNSS_LOC_TOPIC="/jmc_auto/localization/msf_gnss"
LIDAR_LOC_TOPIC="/jmc_auto/localization/msf_lidar" 
FUSION_LOC_TOPIC="/jmc_auto/localization/pose"
ODOMETRY_LOC_TOPIC="/jmc_auto/sensor/gnss/odometry"
CLOUD_TOPIC="/jmc_auto/sensor/velodyne64/compensator/PointCloud2"
...
function data_exporter() {
  local BAG_FILE=$1
  local OUT_FOLDER=$2
  $JMC_AUTO_BIN_PREFIX/modules/localization/msf/local_tool/data_extraction/monitor_data_exporter \
    --bag_file $BAG_FILE \
...
function compare_poses() {
  local IN_FOLDER=$1
  $JMC_AUTO_BIN_PREFIX/modules/localization/msf/local_tool/data_extraction/compare_poses \
    --in_folder $IN_FOLDER \
...
python ${JMC_AUTO_ROOT_DIR}/modules/tools/localization/evaluate_compare.py compare_fusion_odometry_all.txt

echo ""
echo "Lidar localization result:"
python ${JMC_AUTO_ROOT_DIR}/modules/tools/localization/evaluate_compare.py compare_lidar_odometry_all.txt

if [ $# -eq 2 ]; then
  echo ""
  echo "GNSS localization result:"
  python ${JMC_AUTO_ROOT_DIR}/modules/tools/localization/evaluate_compare.py\
