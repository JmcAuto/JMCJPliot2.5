...
function start() {
    # Setup supervisord.
    if [ "$USER" == "release" ]; then
        supervisord -c /jmcauto/modules/tools/supervisord/release.conf >& /tmp/supervisord.start.log
        echo "Started supervisord with release conf"
    else
        supervisord -c /jmcauto/modules/tools/supervisord/dev.conf >& /tmp/supervisord.start.log
    ...
    if [ $? -eq 0 ]; then
        echo "Dreamview is running at http://localhost:${DOCKER_DV_PORTS}"
    fi
}
